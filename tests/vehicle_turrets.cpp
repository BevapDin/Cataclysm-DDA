...
TEST_CASE( "vehicle_turret", "[vehicle] [gun] [magazine] [.]" )
{
    for( auto e : turret_types() ) {
        SECTION( e->name() ) {
            ...
            const auto gunref = veh->install_part( 0, 0, e->get_id(), true );
            REQUIRE( gunref );
            const int idx = gunref->part_index();
            REQUIRE( idx >= 0 );

            REQUIRE( veh->install_part( 0,  0, vpart_id( "storage_battery" ), true ) );
            ...
            if( veh->part_flag( idx, "USE_TANKS" ) ) {
                ...
                const auto tankref = veh->install_part( 0, 0, tank->get_id(), true );
                REQUIRE( tankref );
                const int tank_idx = tankref->part_index();
                REQUIRE( tank_idx >= 0 );
