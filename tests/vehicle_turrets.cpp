...
TEST_CASE( "vehicle_turret", "[vehicle] [gun] [magazine] [.]" )
{
    for( auto e : turret_types() ) {
        SECTION( e->name() ) {
            ...
            const int idx = veh->install_part( tripoint( 0, 0, 0 ), e->get_id(), true );
            REQUIRE( idx >= 0 );

            REQUIRE( veh->install_part( tripoint( 0, 0, 0 ), vpart_id( "storage_battery" ), true ) >= 0 );
            ...
            if( veh->part_flag( idx, "USE_TANKS" ) ) {
                ...
                auto tank_idx = veh->install_part( tripoint( 0, 0, 0 ), tank->get_id(), true );
                REQUIRE( tank_idx >= 0 );
