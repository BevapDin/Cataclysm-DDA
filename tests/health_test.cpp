...
static void test_diet( const time_duration dur, npc &dude, const std::array<int, 4> hmod_changes_per_day,
                       int min, int max )
{
    std::vector<int> health_samples;
    for( time_duration i = 0_turns; i < dur; i += 6_hours ) {
        // 4 updates per day
        const size_t index = to_hours<int>( i % 1_days ) / 6;
