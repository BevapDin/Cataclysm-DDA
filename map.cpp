...
void map::board_vehicle(game *g, int x, int y, player *p)
{
 ...
 const int seat_part = veh->part_with_flag (part, vpf_boardable);
 if (part < 0) {
...
void map::unboard_vehicle(game *g, const int x, const int y)
{
 ...
 const int seat_part = veh->part_with_flag (part, vpf_boardable, false);
 if (part < 0) {
...
bool map::vehproceed(game* g){
   ...
   { // sink in water?
      ...
      for (int ep = 0; ep < veh->external_parts.size(); ep++) {
         ...
         if (veh->part_function(p, vpc_wheel)){
            num_wheels++;
   ...
   if (imp > 0) { // imp == impulse from collisions
      ...
      for (int ps = 0; ps < ppl.size(); ps++) {
         ...
         const int psblt = veh->part_with_function(ppl[ps], vpc_seatbelt);
         ...
         if (throw_from_seat) {
            ...
            g->fling_player_or_monster(psg, 0, mdir.dir() + rng(0, 60) - 30,
                  (vel2/100 - sb_bonus < 10 ? 10 :
                   vel2/100 - sb_bonus));
         } else if (veh->part_with_function (ppl[ps], vpc_controls) >= 0) {
   ...
   if (can_move) {
      for (int ep = 0; ep < veh->external_parts.size(); ep++) {
         const int p = veh->external_parts[ep];
         if (veh->part_function(p, vpc_wheel) && one_in(2))
...
int map::move_cost(const int x, const int y)
{
 ...
 if (veh) {  // moving past vehicle cost
  const int dpart = veh->part_with_flag(vpart, vpf_obstacle);
  if (dpart >= 0 && (!veh->part_flag(dpart, vpf_openable) || !veh->parts[dpart].open)) {
   return 0;
  } else {
    const int ipart = veh->part_with_function(vpart, vpc_aisle);
    if (ipart >= 0)
      return 2;
   return 8;
  }
...
bool map::trans(const int x, const int y)
{
 ...
 if (veh) {
  ...
  if (!tertr) {
   const int dpart = veh->part_with_flag(vpart, vpf_openable);
...
bool map::has_flag(const t_flag flag, const int x, const int y)
{
 if (flag == bashable) {
  ...
  if (veh && veh->parts[vpart].hp > 0 && // if there's a vehicle part here...
      veh->part_with_flag(vpart, vpf_obstacle) >= 0) {// & it is obstacle...
   const int p = veh->part_with_flag(vpart, vpf_openable);
                ...
                const bool hit_items, const std::set<std::string>& ammo_effects)
{
   ...
   // {
    ...
    for (int i = 0; i < i_at(x, y).size(); i++)
    {
        ...
        {
            i_at(x, y)[i].increase_damaged(1);
        }
        if (i_at(x, y)[i].is_destroyed())
        {
            destroyed = true;
        }
                     ...
                     const int quantity, const int charges, const int damlevel)
{
    ...
    // bounds checking for damage level (is now done in set_damaged)
    new_item.set_damaged(damlevel);
  ...
  for (std::vector<int>::iterator part = vehs[v].v->external_parts.begin();
   ...
   if(INBOUNDS(px, py)) {
    ...
    if (vehs[v].v->part_flag(*part, vpf_opaque) && vehs[v].v->parts[*part].hp > 0) {
     int dpart = vehs[v].v->part_with_flag(*part , vpf_openable);
