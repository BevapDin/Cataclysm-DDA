...
int player::calc_focus_equilibrium()
{
    ...
    if (activity.type == ACT_READ)
    {
        ...
        if (reading != NULL && skillLevel(reading->type) < (int)reading->level)
        {
            focus_gain_rate -= 50;
        }
    }
...
// Start by checking weapon contents
 ...
 if (weapon.matches_type(it) || weapon.ammo_type() == it) {
...
void player::consume_effects(item *eaten, it_comest *comest, bool rotten)
{
    ...
    if (has_trait("GOURMAND")) {
        if (fun < -2) {
            add_morale(MORALE_FOOD_BAD, fun * 2, fun * 4, 60, 30, false, comest);
        } else if (fun > 0) {
