...
std::vector<Cursor> Cursor::get_arguments() const
{
    ...
    for( int i = 0; i < n; ++i ) {
        result.emplace_back( clang_Cursor_getArgument( cursor_, i ) );
    }
    return result;
}
...
Cursor Cursor::get_definition() const {
    return clang_getCursorDefinition( cursor_ );
}

Type Cursor::get_result_type() const {
return clang_getCursorResultType( cursor_ );
}

Type Cursor::get_underlying_type() const {
return clang_getTypedefDeclUnderlyingType( cursor_ );
}
bool Cursor::is_definition() const {
    return clang_isCursorDefinition( cursor_);
}

static inline CXChildVisitResult static_visitor( CXCursor cursor, CXCursor parent,CXClientData data )
{
    return ( *static_cast<Cursor::VisitorType *>( data ) )( cursor, parent );
}

void Cursor::visit_children( const VisitorType &func ) const
{
    clang_visitChildren( cursor_, &static_visitor,static_cast<CXClientData>( const_cast<VisitorType *>( &func ) ) );
}
...
void dump( std::ostream &stream, const std::string &msg, const Cursor &c, size_t i, std::set<Cursor> &cursors,std::set<Type> &types);

void dump( std::ostream &stream, const std::string &msg, const Type &t, size_t i, std::set<Cursor> &cursors,std::set<Type> &types) {
...
{
    //@todo
    return std::memcmp( &cursor_, &rhs.cursor_, sizeof( CXCursor ) ) < 0;
}
