...
void iexamine::gunsafe_el(player &p, const tripoint &examp)
{
    ...
    switch( hack_attempt( p ) ) {
        case HACK_FAIL:
            ...
            if (examp.z > 0 && !event_manager::get_instance().queued(EVENT_WANTED)) {
...
void iexamine::pedestal_wyrm(player &p, const tripoint &examp)
{
    ...
    event_manager::get_instance().add(EVENT_SPAWN_WYRMS, int(calendar::turn) + rng(5, 10));
}
...
void iexamine::pedestal_temple(player &p, const tripoint &examp)
{
    ...
    if (g->m.i_at(examp).size() == 1 &&
        ...
        g->m.i_clear(examp);
        event_manager::get_instance().add(EVENT_TEMPLE_OPEN, int(calendar::turn) + 4);
...
void iexamine::fswitch(player &p, const tripoint &examp)
{
    ...
    event_manager::get_instance().add(EVENT_TEMPLE_SPAWN, calendar::turn + 3);
}
