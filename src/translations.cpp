...
void set_language()
{
    ...
    auto env = getenv( "LANGUAGE" );
    locale_dir = FILENAMES["base_path"].native() + "lang/mo/" + ( env ? env : "none" ) +
    ...
    if( !FILENAMES["base_path"].native().empty() ) {
        ...
        locale_dir = cata::path( "lang/mo" );
    }
#else
    locale_dir = cata::path( "lang/mo" );
#endif

    bindtextdomain( "cataclysm-dda", locale_dir.c_str() );
