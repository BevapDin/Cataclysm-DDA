...
std::string item::info(bool showtext, std::vector<iteminfo> *dump, bool debug) const
{
    ...
    for(itype::FunctionalityMap::const_iterator a = type->functionalityMap.begin(); a != type->functionalityMap.end(); ++a) {
        std::ostringstream buffer;
        buffer << "Works as " << (item_controller->has_template(a->first) ? nname(a->first) : a->first);
        std::string std_name = (item_controller->has_template(a->first.substr(5)) ? nname(a->first.substr(5)) : a->first.substr(5));
...

    {
        ...
        for(auto a = recipes.begin(); a != recipes.end(); ++a) {
            ...
            for(auto b = recipes.begin(); b != recipes.end(); ++b) {
                ...
                const auto &comps = r.requirements.components;
                const auto &tools = r.requirements.tools;
...
long item::get_charges_of(const itype_id &type_) const {
    ...
    return static_cast<long>(charges / type->getChargesModi(type_));
}
...
int item::amount_of(const itype_id &it, bool used_as_tool) const
{
    ...
    if (matches_type(it) && (used_as_tool || !has_flag("PSEUDO"))) {
        if (contents.empty()) {
...
bool item::use_charges(const itype_id &it, long &quantity, std::list<item> &used)
{
    ...
    // Now check the item itself
    if (!(matches_type(it) || (is_tool() && (dynamic_cast<it_tool *>(type))->subtype == it)) ||
