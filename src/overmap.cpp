...
point overmap::display_notes(int z)
{
    ...
    catacurses::window w_notes( FULL_SCREEN_HEIGHT, FULL_SCREEN_WIDTH,
...
{
    g->w_omlegend = catacurses::window( TERMY, 28, 0, TERMX - 28 );
    g->w_overmap = catacurses::window( OVERMAP_WINDOW_HEIGHT, OVERMAP_WINDOW_WIDTH, 0, 0 );

    // Draw black padding space to avoid gap between map and legend
    // also clears the pixel minimap in TILES
    g->w_blackspace = catacurses::window( TERMY, TERMX, 0, 0 );
    ...
    do {
        ...
        if (ictxt.get_direction(dirx, diry, action)) {
            ...
            tripoint tmp = curs;
            catacurses::window w_search(13, 27, 3, TERMX - 27);
            ...
            if( pmenu.ret >= 0 ) {
                catacurses::window w_editor( 15, 27, 3, TERMX - 27 );
