...
int requirements::print_components( WINDOW *w, int ypos, int xpos, int width, nc_color col,
                                    const crafting_inventory_t &crafting_inv ) const
{
    if( components.empty() ) {
        return 0;
    }
...
bool requirements::can_make_with_inventory( const crafting_inventory_t &crafting_inv ) const
{
    bool retval = true;
...
bool quality_requirement::has( const crafting_inventory_t &crafting_inv ) const
{
    return crafting_inv.has_items_with_quality( type, level, count );
}

std::string quality_requirement::get_color( bool, const crafting_inventory_t & ) const
{
    return available == a_true ? "green" : "red";
}

bool tool_comp::has( const crafting_inventory_t &crafting_inv ) const
{
...
std::string tool_comp::get_color( bool has_one, const crafting_inventory_t &crafting_inv ) const
{
    if( type == "goggles_welding" ) {
        if( g->u.has_bionic( "bio_sunglasses" ) || g->u.is_wearing( "rm13_armor_on" ) ) {
            return "cyan";
...
bool item_comp::has( const crafting_inventory_t &crafting_inv ) const
{
...
std::string item_comp::get_color( bool has_one, const crafting_inventory_t &crafting_inv ) const
{
    if( type == "rope_30" || type == "rope_6" ) {
        if( g->u.has_trait( "WEB_ROPE" ) && g->u.hunger <= 300 ) {
...
bool requirements::check_enough_materials( const crafting_inventory_t &crafting_inv ) const
{
    bool retval = true;
...
bool requirements::check_enough_materials( const item_comp& comp, const crafting_inventory_t& crafting_inv ) const
{
    if( comp.available != a_true ) {
        return false;
    }
