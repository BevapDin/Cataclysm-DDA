...
std::vector<std::string> requirement_data::get_folded_components_list( int width, nc_color col, const crafting_inventory_t &crafting_inv, int batch) const
{
...
std::vector<std::string> requirement_data::get_folded_tools_list(int width, nc_color col, const crafting_inventory_t &crafting_inv, int batch) const
{
...
bool quality_requirement::has( const crafting_inventory_t &crafting_inv, int ) const
{
    return crafting_inv.has_items_with_quality( type, level, count );
}

std::string quality_requirement::get_color( bool, const crafting_inventory_t &, int ) const
{
    return available == a_true ? "green" : "red";
}

bool tool_comp::has( const crafting_inventory_t &crafting_inv, int batch ) const
{
...
std::string tool_comp::get_color( bool has_one, const crafting_inventory_t &crafting_inv, int batch ) const
{
    if( type == "goggles_welding" ) {
        if( g->u.has_bionic( "bio_sunglasses" ) || g->u.is_wearing( "rm13_armor_on" ) ) {
            return "cyan";
...
bool item_comp::has( const crafting_inventory_t &crafting_inv, int batch ) const
{
...
std::string item_comp::get_color( bool has_one, const crafting_inventory_t &crafting_inv, int batch ) const
{
    if( type == "rope_30" || type == "rope_6" ) {
        if( g->u.has_trait( "WEB_ROPE" ) && g->u.hunger <= 300 ) {
