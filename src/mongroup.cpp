...
const MonsterGroup &MonsterGroupManager::GetUpgradedMonsterGroup( const mongroup_id &group )
{
    ...
    if( get_option<float>( "MONSTER_UPGRADE_FACTOR" ) > 0 ) {
        const time_duration replace_time = groupptr->monster_group_time * get_option<float>( "MONSTER_UPGRADE_FACTOR" ) );
        while( groupptr->replace_monster_group && calendar::turn - calendar::time_of_cataclysm > replace_time ) {
