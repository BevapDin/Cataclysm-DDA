...
int player::calc_focus_equilibrium()
{
    ...
    if (activity.type == ACT_READ) {
        ...
        if( book.is_book() ) {
            ...
            if (skillLevel(book.type->book_slot->type) < (int)book.type->book_slot->level) {
                focus_gain_rate -= 50;
            }
        } else {
        ...
        if (which == -1) {
            ...
            if (!is_npc()) {
                ...
                if (OPTIONS["DROP_EMPTY"] == "no") {
                    ...
                    drop_it = it.is_container() && !it.is_watertight();
...
hint_rating player::rate_action_wear(item *it)
{
    ...
    if (worn.size() && it->is_power_armor() && !(it->covers.test(bp_head))) {
        if (!(it->covers.test(bp_torso) && it->type->color == c_green)) {
            return HINT_IFFY;
        }
    }
...
bool player::wear_item(item *to_wear, bool interactive)
{
    ...
    if (to_wear->is_power_armor())
    {
        ...
        for (auto &i : worn)
        {
            if (i.is_power_armor() && i.type == to_wear->type)
            {
                if(interactive)
                {
    ...
    if (!to_wear->has_flag("OVERSIZE")) {
        ...
        if ((to_wear->covers.test(bp_hand_l) || to_wear->covers.test(bp_hand_r)) && has_trait("WEBBED"))
        {
            if(interactive)
            {
                add_msg(m_info, _("You cannot put %s over your webbed hands."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
              ...
              has_trait("ARM_TENTACLES_8")) )
        {
            if(interactive)
            {
                add_msg(m_info, _("You cannot put %s over your tentacles."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
        ...
        if ((to_wear->covers.test(bp_hand_l) || to_wear->covers.test(bp_hand_r)) && has_trait("TALONS"))
        {
            if(interactive)
            {
                add_msg(m_info, _("You cannot put %s over your talons."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
        ...
        if ((to_wear->covers.test(bp_hand_l) || to_wear->covers.test(bp_hand_r)) && (has_trait("PAWS") || has_trait("PAWS_LARGE")) )
        {
            if(interactive)
            {
                add_msg(m_info, _("You cannot get %s to stay on your paws."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
        ...
        has_trait("BEAK_HUM")) )
        {
            if(interactive)
            {
                add_msg(m_info, _("You cannot put a %s over your beak."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
            ...
            (has_trait("MUZZLE") || has_trait("MUZZLE_BEAR") || has_trait("MUZZLE_LONG")))
        {
            if(interactive)
            {
                add_msg(m_info, _("You cannot fit the %s over your muzzle."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
        ...
        if (to_wear->covers.test(bp_mouth) && has_trait("MINOTAUR"))
        {
            if(interactive)
            {
                add_msg(m_info, _("You cannot fit the %s over your snout."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
        ...
        if (to_wear->covers.test(bp_mouth) && has_trait("SABER_TEETH"))
        {
            if(interactive)
            {
                add_msg(m_info, _("Your saber teeth are simply too large for %s to fit."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
        ...
        if (to_wear->covers.test(bp_mouth) && has_trait("MANDIBLES"))
        {
            if(interactive)
            {
                add_msg(_("Your mandibles are simply too large for %s to fit."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
        ...
        if (to_wear->covers.test(bp_mouth) && has_trait("PROBOSCIS"))
        {
            if(interactive)
            {
                add_msg(m_info, _("Your proboscis is simply too large for %s to fit."), to_wear->type->nname(1).c_str());
            }
            return false;
        }
        ...
        if (it->charges == it->clip_size()) {
            ...
            for (auto &i : it->contents) {
                if ((i.is_gunmod() && (i.typeId() == "spare_mag" &&
                      i.charges < it->type->gun_slot->clip)) ||
...
void player::use(int pos)
{
 ...
 can modify weapons."));
        ...
        islot_gun* guntype = gun->type->gun_slot.get();
...
hint_rating player::rate_action_read(item *it)
{
 ...
 if (g && g->light_level() < 8 && LL_LIT > g->m.light_at(posx, posy)) {
  return HINT_IFFY;
 } else if (morale_level() < MIN_MORALE_READ &&  it->type->book_slot->fun <= 0) {
  return HINT_IFFY; //won't read non-fun books when sad
 } else if (it->type->book_slot->intel > 0 && has_trait("ILLITERATE")) {
  return HINT_IFFY;
...
// Some macguffins can be read, but they aren't treated like books.
    ...
    auto tmp = it->type->book_slot.get();
...
{
    auto reading = book->type->book_slot.get();

    if( !has_identified( book->type->id ) ) {
        // Note that we've read the book.
        items_identified.insert( book->type->id );

        add_msg(_("You skim %s to find out what's in it."), book->tname().c_str());
        if( reading->type ) {
    ...
    if( skillLevel(reading->type) < (int)reading->level ) {
        ...
        if (skillLevel(reading->type) == (int)reading->level) {
            if (no_recipes) {
                add_msg(m_info, _("You can no longer learn from %s."), book->type->nname(1).c_str());
            } else {
                add_msg(m_info, _("Your skill level won't improve, but %s has more recipes for you."),
                        book->type->nname(1).c_str());
            }
        }
    }
...
{
    for( auto &rv : book->book_slot->recipes ) {
        if( !knows_recipe( rv.first ) &&
            ( rv.first->skill_used == nullptr || skillLevel( rv.first->skill_used ) >= rv.second ) ) {
            return true;
        }
    }
    return false;
...
{
    for( auto &rv : book->book_slot->recipes ) {
        const recipe *r = rv.first;
        if( knows_recipe( r ) ) {
            continue;
        }
        if( r->skill_used != nullptr && skillLevel( r->skill_used ) < rv.second ) {
            continue;
        }
        const int cmp_lev = ( skillLevel( r->skill_used ) - rv.second ) / 2;
        if( r->skill_used == nullptr || rng( 0, 4 ) <= cmp_lev ) {
            learn_recipe( r );
            add_msg( m_good, _( "Learned a recipe for %s from the %s." ),
                     item_controller->find_template( r->result )->nname( 1 ).c_str(), book->nname( 1 ).c_str() );
            return true;
        } else {
...
std::string player::is_snuggling()
{
    ...
    const itype* floor_armor = nullptr;
    int ticker = 0;
...
bool player::armor_absorb(damage_unit& du, item& armor) {
    ...
    std::string pre_damage_adj = armor.type->dmg_adj(armor.damage);
    ...
    if (rng(0,100) <= armor.get_coverage()) {
        ...
        if ((du.amount > effective_resist && !one_in(du.amount) && one_in(2)) ||
            ...
            std::string damage_verb = du.type == DT_BASH
                ? armor.type->bash_dmg_verb()
                : armor.type->cut_dmg_verb();
                ...
                if (worn[i].is_power_armor()) {
                    ...
                    if (worn[i].damage >= 5) {
                      ...
                      add_memorial_log(pgettext("memorial_male", "Worn %s was completely destroyed."),
                        ...
                        std::string damage_verb = diff_bash > diff_cut ? worn[i].type->bash_dmg_verb() :
                                                                         worn[i].type->cut_dmg_verb();
...
int player::get_env_resist(body_part bp) const
{
    ...
    for (auto &i : worn) {
        ...
        if (i.covers.test(bp) || (bp == bp_eyes && i.covers.test(bp_head))) {
            ret += i.type->armor_slot->env_resist;
        }
    }
...
bool player::is_wearing_power_armor(bool *hasHelmet) const {
    bool result = false;
    for (size_t i = 0; i < worn.size(); i++) {
        if( !worn[i].is_armor() || !worn[i].is_power_armor()) {
            continue;
        }
        if (hasHelmet == NULL) {
...
int player::has_recipe( const recipe *r, const inventory &crafting_inv ) const
{
    ...
    for( auto stack = slice.cbegin(); stack != slice.cend(); ++stack ) {
        ...
        if( candidate.is_book() && items_identified.count(candidate.type->id) ) {
            for( auto &rv : candidate.type->book_slot->recipes ) {
                // Does it have the recipe, and do we meet it's requirements?
                if( rv.first->ident == r->ident &&
                    ( rv.first->skill_used == NULL ||
                      get_skill_level(rv.first->skill_used) >= rv.second ) &&
                    ( difficulty == -1 || rv.second < difficulty ) ) {
                    difficulty = rv.second;
                }
            }
        } else {
...
{
    // TODO: the map should refer to const recipes, this cast should not be needed.
    learned_recipes[rec->ident] = const_cast<recipe*>( rec );
}
