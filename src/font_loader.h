...
class font_loader
{
    ...
    private:
        void load_throws( const cata::path &path ) {
            try {
                ...
                throw std::runtime_error( std::string( "loading font settings from " ) + path.string() + " failed: "
                                          +
                                          err.what() );
            }
        }
        void save( const cata::path &path ) const {
