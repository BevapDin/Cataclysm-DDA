...
{
    if( renderer == NULL ) {
        dbg( D_ERROR ) << "cata_tiles: renderer is NULL!";
    }
    ...
    last_pos = tripoint( INT_MIN, INT_MIN, INT_MIN );
}
...
bool cata_tiles::draw_from_id_string(const std::string &id, TILE_CATEGORY category, const std::string &subcategory, int x, int y, int subtile, int rota)
{
    ...
    if (x - o_x < 0 || x - o_x >= screen_tile_count_x || y - o_y < 0 || y - o_y >= screen_tile_count_y) {
        return false;
    }
...
void cata_tiles::apply_changes(int destx, int desty, int width, int height) {
    ...
    const point newpos2D = g->m.getabs( g->u.posx, g->u.posy );
    const tripoint newpos( newpos2D.x, newpos2D.y, g->levz );
    const int delta_pos_x = newpos.x - last_pos.x;
    const int delta_pos_y = newpos.y - last_pos.y;
    if( newpos.z == last_pos.z ) {
        scroll( delta_pos_x, delta_pos_y, width, height );
    }
    last_pos = newpos;
