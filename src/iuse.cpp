...
int iuse::simple_off(player *p, item *it, bool t)
{
 if (it->charges == 0)
  ...
  on_type += "on";
  it->make(itypes[on_type]);
  it->active = true;
 }
 return it->type->charges_to_use();
}

int iuse::simple_on(player *p, item *it, bool t)
{
 if (t) {    // Normal use
// Do nothing...
 } else {    // Turning it off
...
int iuse::mine_bomb(player *p, item *it, bool t)
{
    g->add_msg_if_player(p,_("You pull the pin on the %s."), it->tname(g).c_str());
    it->make(itypes[it->type->id + "_act"]);
    it->charges = 30;
    it->active = true;
    return it->type->charges_to_use();
}

int iuse::mine_bomb_act(player *p, item *it, bool t)
{
    point pos = g->find_item(it);
    if (pos.x == -999 || pos.y == -999) {
        return 0;
...
int iuse::tool_belt(player *p, item *it, bool t)
{
...
int iuse::quiver(player *p, item *it, bool t)
{
...
int iuse::wood_gas(player *p, item *, bool t)
{
    ...
    p->moves -= 30 * vol_fac;
    item gasoline(itypes["gasoline"], g->turn);
