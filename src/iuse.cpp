...
int iuse::tool_belt(player *p, item *it, bool, const tripoint &)
{
    ...
    menu.query();
    if( menu.ret == -10 ) {
        int pos = g->inv_for_filter(_("Put what?"), [](const item &itm) {
            return itm.is_tool() && itm.volume() <= 3;
        });
        item* put = &(p->i_at(pos));
        if(put->is_null()) {
            p->add_msg_if_player(_("You do not have that item!"));
    ...
    if( index < it->contents.size() ) {
        ...
        p->use( pos );
        std::list<item> backs = p->remove_items_with( [](const item &it) { return it.mission_id == -100; } );
        if( !backs.empty() ) {
...
int iuse::wood_gas(player *p, item *, bool, const tripoint & )
{
