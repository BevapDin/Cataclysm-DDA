...
player &iuse_invocation_data::user() const
{
    if( !p ) {
        debugmsg( "Tried to get player reference from iuse_invocation_data when there was none for item %s",
                  it.tname() );
        throw std::runtime_error( "non-existing player reference requested" );
    }
    return *p;
}

int iuse_invocation_data::charges_to_use() const
{
    return data.charges_to_use();
}
 ...
 */
int iuse::sewage( const iuse_invocation_data &data )
{
    player &p = p.user();
...
int iuse::honeycomb( const iuse_invocation_data &data )
{
    g->m.spawn_item( data.user().pos(), "wax", 2 );
    return data.charges_to_use();
}

int iuse::royal_jelly( const iuse_invocation_data &data )
{
    data.user().add_effect( effect_cureall, 1);
    return data.charges_to_use();
}

int iuse::xanax( const iuse_invocation_data &data )
{
    data.user().add_msg_if_player(_("You take some %s."), it.tname().c_str());
    data.user().add_effect( effect_took_xanax, 900);
    return data.charges_to_use();
}

int iuse::caff( const iuse_invocation_data &data )
{
    data.user().mod_fatigue( -( it.type->comestible ? it.type->comestible->stim : 0 ) * 3 );
    return data.charges_to_use();
}

int iuse::atomic_caff( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::alcohol_weak( const iuse_invocation_data &data )
{
    return alcohol( data.user_ptr(), &data.item(), 0 );
}

int iuse::alcohol_medium( const iuse_invocation_data &data )
{
    return alcohol( data.user_ptr(), &data.item(), 1);
}

int iuse::alcohol_strong( const iuse_invocation_data &data )
{
    return alcohol( data.user_ptr(), &data.item(), 2);
}
 ...
 */
int iuse::smoking( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::ecig( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (it.typeId() == "ecig") {
...
int iuse::antibiotic( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::eyedrops( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
    ...
    if( it.charges < data.charges_to_use() ) {
...
int iuse::fungicide( const iuse_invocation_data &data )
{
    player &p = data.user();
    if (p.is_underwater()) {
        p.add_msg_if_player(m_info, _("You can't do that while underwater."));
...
int iuse::antifungal( const iuse_invocation_data &data )
{
    player &p = data.user();
    if (p.is_underwater()) {
        p.add_msg_if_player(m_info, _("You can't do that while underwater."));
...
int iuse::antiparasitic( const iuse_invocation_data &data )
{
    player &p = data.user();
    if (p.is_underwater()) {
        p.add_msg_if_player(m_info, _("You can't do that while underwater."));
...
int iuse::anticonvulsant( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::weed_brownie( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::coke( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::meth( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::vaccine( const iuse_invocation_data &data )
{
    player &p = data.user();
    ...
    p.mod_pain(3);
    item syringe( "syringe", data.item().birthday() );
    p.i_add( syringe );
    return data.charges_to_use();
}

int iuse::flu_vaccine( const iuse_invocation_data &data )
{
    player &p = data.user();
    p.add_msg_if_player(_("You inject the vaccine."));
...
int iuse::poison( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
 ...
 * for magic mushrooms).
 */
int iuse::fun_hallu( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::meditate( const iuse_invocation_data &data )
{
    if( !data.has_user() || data.is_active() ) {
        return 0;
    }
...
int iuse::thorazine( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    p.mod_fatigue(5);
...
int iuse::prozac( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::sleep( const iuse_invocation_data &data )
{
    player &p = data.user();
    p.mod_fatigue(40);
...
int iuse::datura( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::flumed( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::flusleep( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::inhaler( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::oxygen_bottle( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    p.moves -= 500;
...
int iuse::blech( const iuse_invocation_data &data )
{
    player &p = data.user();
...
int iuse::plantblech( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::chew( const iuse_invocation_data &data )
{
    // TODO: Add more effects?
    data.user().add_msg_if_player( _( "You chew your %s." ), data.item().tname() );
    return data.charges_to_use();
}
...
int iuse::mutagen( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::mut_iv( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    mutagen_attempt checks = mutagen_common_checks( p, it, false,
...
int iuse::purifier( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    mutagen_attempt checks = mutagen_common_checks( p, it, false,
...
int iuse::purify_iv( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    mutagen_attempt checks = mutagen_common_checks( p, it, false,
...
int iuse::marloss( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::marloss_seed( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::marloss_gel( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( marloss_prevented( p ) ) {
...
int iuse::mycus( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_npc()) {
...
int iuse::dogfood( const iuse_invocation_data &data )
{
    return petfood( data.user(), data.item(), DOGFOOD );
}

int iuse::catfood( const iuse_invocation_data &data )
{
    return petfood( data.user(), data.item(), CATFOOD );
}

int iuse::feedcattle( const iuse_invocation_data &data )
{
    return petfood( data.user(), data.item(), CATTLEFODDER );
}

int iuse::sew_advanced( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::radio_mod( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::remove_all_mods( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !p ) {
...
int iuse::fishing_rod( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::fish_trap( const iuse_invocation_data &data )
{
    item &it = data.item();
    if( !data.is_active() ) {
        player &p = data.user();
...
int iuse::extinguisher( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::rm13armor_off( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::rm13armor_on( const iuse_invocation_data &data )
{
    if (data.is_active()) { // Normal use
    } else { // Turning it off
        player &p = data.user();
        item &it = data.item();
...
int iuse::unpack_item( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
int iuse::pack_item( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
static int cauterize_elec(player &p, item &it)
{
    if (it.charges == 0 && it.ammo_capacity()) {
        ...
        if ((p.has_trait( trait_MASOCHIST ) || p.has_trait( trait_MASOCHIST_MED ) || p.has_trait( trait_CENOBITE )) &&
            ...
            return cauterize_actor::cauterize_effect( *p, *it, true ) ? data.charges_to_use() : 0;
        } else {
...
int iuse::water_purifier( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::radio_off( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::directional_antenna( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::radio_on(  const iuse_invocation_data &data  )
{
    if( data.is_active() ) {
        // Normal use
...
int iuse::noise_emitter_off( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::noise_emitter_on( const iuse_invocation_data &data )
{
    if (data.is_active()) { // Normal use
        //~ the sound of a noise emitter when turned on
        sounds::ambient_sound(data.position(), 30, _("KXSHHHHRRCRKLKKK!"));
    } else { // Turning it off
        data.user().add_msg_if_player(_("The infernal racket dies as the noise emitter turns off."));
        data.item().convert( "noise_emitter" ).active = false;
    }
    return data.charges_to_use();
}

int iuse::ma_manual( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
bool pry_nails( player &p, const ter_id &type, const int dirx, const int diry )
{
...
int iuse::hammer( const iuse_invocation_data &data )
{
    player &p = data.user();
    g->draw();
    int x, y;
...
int iuse::crowbar( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::makemound( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::dig( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    for( const tripoint &pt : closest_tripoints_first( 1, data.position() ) ) {
...
int iuse::siphon( const iuse_invocation_data &data )
{
    tripoint posp;
    ...
    if (veh == NULL) {
        data.user().add_msg_if_player(m_info, _("There's no vehicle there."));
        return 0;
    }
...
int iuse::combatsaw_off( const iuse_invocation_data &data )
{
    return toolweapon_off( data.user(), data.item(),
        true,
        !data.user().is_underwater(),
        30, _("With a snarl, the combat chainsaw screams to life!"),
        _("You yank the cord, but nothing happens."));
}

int iuse::chainsaw_off( const iuse_invocation_data &data )
{
    return toolweapon_off( data.user(), data.item(),
        false,
        rng(0, 10) - data.item().damage() > 5 && !data.user().is_underwater(),
        20, _("With a roar, the chainsaw leaps to life!"),
        _("You yank the cord, but nothing happens."));
}

int iuse::elec_chainsaw_off( const iuse_invocation_data &data )
{
    return toolweapon_off(data.user(), data.item(),
        false,
        rng(0, 10) - data.item().damage() > 5 && !data.user().is_underwater(),
...
int iuse::cs_lajatang_off( const iuse_invocation_data &data )
{
    item &it = data.item();
    return toolweapon_off(data.user(), it,
        false,
        rng(0, 10) - it.damage() > 5 && it.ammo_remaining() > 1 && !data.user().is_underwater(),
        40, _("With a roar, the chainsaws leap to life!"),
        _("You yank the cords, but nothing happens."));
}

int iuse::carver_off( const iuse_invocation_data &data )
{
    return toolweapon_off(data.user(), data.item(),
        false,
        true,
...
int iuse::trimmer_off( const iuse_invocation_data &data )
{
    return toolweapon_off(data.user(), data.item(),
        false,
        rng(0, 10) - data.item().damage() > 3,
...
int iuse::combatsaw_on( const iuse_invocation_data &data )
{
    return toolweapon_on(data.user(), data.item(), data.is_active(), _("combat chainsaw"),
        false,
        12, 18, _("Your combat chainsaw growls."));
}

int iuse::chainsaw_on( const iuse_invocation_data &data )
{
    return toolweapon_on(data.user(), data.item(), data.is_active(), _("chainsaw"),
        false,
        15, 12, _("Your chainsaw rumbles."));
}

int iuse::elec_chainsaw_on( const iuse_invocation_data &data )
{
    return toolweapon_on(data.user(), data.item(), data.is_active(), _("electric chainsaw"),
        false,
        15, 12, _("Your electric chainsaw rumbles."));
}

int iuse::cs_lajatang_on( const iuse_invocation_data &data )
{
    return toolweapon_on(data.user(), data.item(), data.is_active(), _("chainsaw lajatang"),
        false,
...
int iuse::carver_on( const iuse_invocation_data &data )
{
    return toolweapon_on(data.user(), data.item(), data.is_active(), _("electric carver"),
        true,
        10, 8, _("Your electric carver buzzes."));
}

int iuse::trimmer_on( const iuse_invocation_data &data )
{
    return toolweapon_on(data.user(), data.item(), data.is_active(), _("hedge trimmer"),
        true,
        15, 10, _("Your hedge trimmer rumbles."));
}

int iuse::circsaw_on( const iuse_invocation_data &data )
{
    return toolweapon_on(data.user(), data.item(), data.is_active()t, _("circular saw"),
        true,
        15, 7, _("Your circular saw buzzes."));
}

int iuse::jackhammer( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::pickaxe( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::geiger( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (data.is_active()) { // Every-turn use when it's on
...
int iuse::teleport( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::can_goo( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::throwable_extinguisher_act( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::pipebomb_act( const iuse_invocation_data &data )
{
    const tripoint &pos = data.position();
    const item &it = data.item();
    if (pos.x == -999 || pos.y == -999) {
        return 0;
    }
    if (data.is_active()) { // Simple timer effects
        //~ the sound of a lit fuse
        sounds::sound(data.position(), 0, _("ssss...")); // Vol 0 = only heard if you hold it
...
int iuse::granade( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::granade_act( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    const tripoint &pos = data.position();
...
int iuse::c4( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    int time;
...
int iuse::acidbomb_act( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !p.has_item( it ) ) {
        tripoint tmp = data.position();
        int &x = tmp.x;
        int &y = tmp.y;
        if (tmp.x == -999) {
            tmp = p.pos();
...
int iuse::grenade_inc_act( const iuse_invocation_data &data )
{
    const tripoint &pos = data.position();
    if (pos.x == -999 || pos.y == -999) {
        return 0;
    ...
    } else if (data.item().charges > 0) {
        data.user().add_msg_if_player(m_info, _("You've already released the handle, try throwing it instead."));
        return 0;
    } else {  // blow up
...
int iuse::arrow_flamable( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
int iuse::molotov_lit( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    const tripoint &pos = data.position();
...
int iuse::firecracker_pack( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
int iuse::firecracker_pack_act( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::firecracker( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
int iuse::firecracker_act( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    const tripoint &pos = data.position();
...
int iuse::mininuke( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    int time;
...
int iuse::pheromone( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::portal( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::tazer( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::tazer2( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::shocktonfa_off( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::shocktonfa_on( const iuse_invocation_data &data )
{
    if (data.is_active()) {  // Effects while simply on
...
int iuse::mp3( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::mp3_on( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (data.is_active()) { // Normal use
        if( p.has_item( it ) ) {
            ...
            play_music( p, data.position(), 0, 20 );
        }
    } else { // Turning it off
...
int iuse::portable_game( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::vibe( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::vortex( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::dog_whistle( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
int iuse::blood_draw( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::lumber( const iuse_invocation_data &data )
{
    if( data.is_active() ) {
        return 0;
    }
...
int iuse::chop_tree(  const iuse_invocation_data &data  )
{
    if( !data.has_user() || data.is_active() ) {
        return 0;
    }

    tripoint dirp = data.position();
...
int iuse::chop_logs(  const iuse_invocation_data &data  )
{
    if( !data.has_user() || data.is_active() ) {
        return 0;
    }
...
int iuse::oxytorch( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::hacksaw(  const iuse_invocation_data &data  )
{
    if( !data.has_user() || data.is_active() ) {
        return 0;
    }
...
int iuse::portable_structure( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::torch_lit( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
int iuse::battletorch_lit( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
int iuse::boltcutters( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    tripoint dirp = pos;
...
int iuse::mop( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    int dirx, diry;
...
int iuse::artifact( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ) {
...
int iuse::spray_can( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::heatpack( const iuse_invocation_data &data )
{
    if( heat_item( data.user() ) ) {
        data.item().convert( "heatpack_used" );
    }
    return 0;
}

int iuse::heat_food( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::hotplate( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::towel( const iuse_invocation_data &data )
{
    if( data.is_active() ) {
...
int iuse::unfold_generic( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
int iuse::adrenaline_injector( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::jet_injector( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::stimpack( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::radglove( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.get_item_position(it) >= -1) {
...
int iuse::contacts( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (p.is_underwater()) {
...
int iuse::talking_doll( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::gun_repair( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::gunmod_attach( const iuse_invocation_data &data ) {
    item &it = data.item();
    if( !it.is_gunmod() ) {
...
int iuse::toolmod_attach( const iuse_invocation_data &data ) {
    item &it = data.item();
    if( !it.is_toolmod() ) {
...
int iuse::misc_repair( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::bell( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::seed( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( p.is_npc() ||
        query_yn(_("Sure you want to eat the %s? You could plant it in a mound of dirt."),
            it.tname().c_str())) {
        return data.charges_to_use(); //This eats the seed object.
    }
    return 0;
}

int iuse::robotcontrol( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
           ...
           const size_t mondex = pick_robot.ret;
            ...
            if (success >= 0) {
                ...
                if( z->is_dead() ) {
                    ...
                    return data.charges_to_use(); // Do not do the other effects if the robot died
                }
                if (one_in(3)) {
...
int iuse::einktabletpc( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (data.is_active()) {
...
int iuse::camera( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::ehandcuffs( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (data.is_active()) {
...
int iuse::radiocar( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    int choice = -1;
...
int iuse::radiocaron( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (data.is_active()) {
...
int iuse::radiocontrol( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if (data.is_active()) {
...
int iuse::remoteveh( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::multicooker( const iuse_invocation_data &data )
{
...
int iuse::cable_attach( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::shavekit( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !it.ammo_sufficient() ) {
...
int iuse::hairkit( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::weather_tool( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::capture_monster_act( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
...
int iuse::ladder( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( !g->m.has_zlevels() ) {
...
int iuse::washclothes( const iuse_invocation_data &data )
{
    player &p = data.user();
    item &it = data.item();
    if( it.charges < data.charges_to_use() ) {
