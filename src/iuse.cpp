...
std::vector<tripoint> points_for_gas_cloud(tripoint center, int radius)
{
...
int iuse::flu_vaccine(player *p, item *it, bool, tripoint)
{
    p->add_msg_if_player(_("You inject the vaccine."));
...
int iuse::meditate(player *p, item *it, bool, tripoint)
{
    if (p->has_trait("SPIRITUAL")) {
        p->moves -= 2000;
...
int iuse::mut_iv(player *p, item *it, bool, tripoint)
{
    ...
    if (it->has_flag("MUTAGEN_STRONG")) {
        ...
        if (!(p->has_trait("NOPAIN"))) {
            p->mod_pain(20);
            sounds::sound(p->pos(), 25 + 3 * p->str_cur, _("You roar in agony!!"));
...
int iuse::remove_all_mods(player *p, item *, bool, tripoint)
{
...
int iuse::elec_chainsaw_off(player *p, item *it, bool, tripoint)
{
    p->moves -= 80;
    if (rng(0, 10) - it->damage > 5 && it->charges > 0 && !p->is_underwater()) {
        sounds::sound(p->pos(), 20,
...
int iuse::elec_chainsaw_on(player *p, item *it, bool t, tripoint)
{
    if (p->is_underwater()) {
        p->add_msg_if_player(_("Your chainsaw gurgles in the water and stops."));
    ...
    switch (choice) {
        case 1: {
            ...
            if (it->charges <= 0) {
                ...
                sounds::sound(p->pos(), 10, _("Let's dance, Zeds!"));
...
int iuse::shishkebab_on(player *p, item *it, bool t, tripoint pos)
{
    if (p->is_underwater()) {
        ...
        if (one_in(25)) {
            sounds::ambient_sound(p->pos(), 10, _("Your shishkebab crackles!"));
        }
    ...
    switch (choice) {
        case 1: {
            ...
            if (rng(0, 10) - it->damage > 2 && it->charges > 0 && !p->is_underwater()) {
                sounds::sound(p->pos(), 10, _("Your No. 9 glows!"));
...
int iuse::firemachete_on(player *p, item *it, bool t, tripoint pos)
{
    if (t) {  // Effects while simply on
        if (p->is_underwater()) {
            ...
            sounds::ambient_sound(p->pos(), 5, _("Your No. 9 hisses."));
        }
        if (one_in(100)) {
...
int iuse::broadfire_off(player *p, item *it, bool t, tripoint pos)
{
    ...
    switch (choice) {
        case 1: {
            p->moves -= 10;
            if (it->charges > 0 && !p->is_underwater()) {
                sounds::sound(p->pos(), 10, _("Charge!!"));
...
int iuse::firekatana_off(player *p, item *it, bool t, tripoint pos)
{
    ...
    switch (choice) {
        case 1: {
            p->moves -= 10;
            if (it->charges > 0 && !p->is_underwater()) {
                sounds::sound(p->pos(), 10, _("The Sun rises."));
...
int iuse::pheromone(player *p, item *it, bool, tripoint pos)
{
    if (it->charges < it->type->charges_to_use()) {
        return 0;
    }
...
int iuse::holster_gun(player *p, item *it, bool, tripoint)
{
...
int iuse::ehandcuffs(player *p, item *it, bool t, tripoint pos)
{
    if (t) {
        ...
        const int z = it->get_var( "HANDCUFFS_Z", 0 );
...
vehicle *pickveh( tripoint center, bool advanced )
{
...
int iuse::remoteveh(player *p, item *it, bool t, tripoint pos)
{
...
int iuse::weather_tool(player *p, item *it, bool, tripoint)
{
