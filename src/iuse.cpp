...
{
    const std::vector<tripoint> gas_sources = closest_points_first(radius, center);
    ...
    for( const auto &p : gas_sources ) {
        ...
        if (p != center) {
            ...
            if (!g->m.clear_path(center, p, radius, 1, 100, junk)) {
...
int iuse::mut_iv(player *p, item *it, bool, const tripoint &)
{
    ...
    if (it->has_flag("MUTAGEN_STRONG")) {
        ...
        if (!(p->has_trait("NOPAIN"))) {
            p->mod_pain(20);
            g->sound(p->pos(), 25 + 3 * p->str_cur, _("You roar in agony!!"));
...
int iuse::resolve_firestarter_use(player *p, item *, const tripoint &pos)
{
...
int iuse::firestarter(player *p, item *it, bool t, const tripoint &)
{
    tripoint pos;
    if (it->has_flag("LENS")) {
        ...
        if( (g->weather == WEATHER_CLEAR || g->weather == WEATHER_SUNNY) &&
            ...
            if( prep_firestarter_use(p, it, pos ) ) {
                ...
                p->activity.placement = point( pos.x, pos.y );
...
int iuse::radio_on(player *p, item *it, bool t, const tripoint &pos)
{
    if (t) {
        ...
        g->ambient_sound(pos, 6, message.c_str());
    } else { // Activated
        int ch = 2;
        if (it->charges > 0) {
...
int iuse::noise_emitter_on(player *p, item *it, bool t, const tripoint &pos)
{
    if (t) { // Normal use
        ...
        g->ambient_sound(pos, 30, _("KXSHHHHRRCRKLKKK!"));
    } else { // Turning it off
...
int iuse::crowbar(player *p, item *it, bool, const tripoint &)
{
    int dirx, diry;
    ...
    if (dice(4, difficulty) < dice(2, p->skillLevel("mechanics")) + dice(2, p->str_cur)) {
        ...
        if (type == t_door_locked_alarm) {
            ...
            g->sound(p->pos(), 40, _("An alarm sounds!"));
...
int iuse::combatsaw_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) { // Effects while simply on
        if (p->is_underwater()) {
            ...
            g->ambient_sound(p->pos(), 18, _("Your combat chainsaw growls."));
        }
    } else { // Toggling
...
int iuse::chainsaw_on(player *p, item *it, bool t, const tripoint &)
{
    if (p->is_underwater()) {
        ...
        if (one_in(15)) {
            g->ambient_sound(p->pos(), 12, _("Your chainsaw rumbles."));
        }
    } else { // Toggling
...
int iuse::cs_lajatang_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) { // Effects while simply on
        if (one_in(15)) {
            g->ambient_sound(p->pos(), 12, _("Your chainsaws rumble."));
        }
...
int iuse::carver_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) { // Effects while simply on
        if (one_in(10)) {
            g->ambient_sound(p->pos(), 8, _("Your electric carver buzzes."));
        }
    } else { // Toggling
...
int iuse::trimmer_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) { // Effects while simply on
        if (one_in(15)) {
            g->ambient_sound(p->pos(), 10, _("Your hedge trimmer rumbles."));
        }
    } else { // Toggling
...
int iuse::circsaw_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) { // Effects while simply on
        if (one_in(15)) {
            g->ambient_sound(p->pos(), 7, _("Your circular saw buzzes."));
        }
    } else { // Toggling
...
int iuse::shishkebab_off(player *p, item *it, bool, const tripoint &pos)
{
    ...
    switch (choice) {
        case 1: {
            ...
            if (it->charges <= 0) {
                ...
                g->sound(p->pos(), 10, _("Let's dance, Zeds!"));
...
int iuse::shishkebab_on(player *p, item *it, bool t, const tripoint &)
{
    if (p->is_underwater()) {
        ...
        if (one_in(25)) {
            g->ambient_sound(p->pos(), 10, _("Your shishkebab crackles!"));
        }
...
int iuse::firemachete_off(player *p, item *it, bool, const tripoint &pos)
{
    ...
    switch (choice) {
        case 1: {
            ...
            if (rng(0, 10) - it->damage > 2 && it->charges > 0 && !p->is_underwater()) {
                g->sound(p->pos(), 10, _("Your No. 9 glows!"));
...
int iuse::firemachete_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) {  // Effects while simply on
        if (p->is_underwater()) {
            ...
            g->ambient_sound(p->pos(), 5, _("Your No. 9 hisses."));
        }
        if (one_in(100)) {
...
int iuse::broadfire_off(player *p, item *it, bool t, const tripoint &pos)
{
...
int iuse::broadfire_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) {  // Effects while simply on
        if (p->is_underwater()) {
...
int iuse::firekatana_off(player *p, item *it, bool t, const tripoint &pos)
{
...
int iuse::firekatana_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) {  // Effects while simply on
        if (p->is_underwater()) {
            p->add_msg_if_player(_("Your sword hisses in the water and goes out."));
...
int iuse::zweifire_off(player *p, item *it, bool t, const tripoint &pos)
{
...
int iuse::zweifire_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) {  // Effects while simply on
        if (p->is_underwater()) {
...
int iuse::set_trap(player *p, item *it, bool, const tripoint &)
{
    ...
    if (existing_trap != tr_null) {
        ...
        if (t.can_see(*p, tripoint(posx, posy, 0))) {
            ...
            t.trigger(p, tripoint(posx, posy, 0));
        }
        return 0;
    }
    ...
    if (!tr->can_see(*p, tripoint(posx, posy, 0))) {
        p->add_known_trap(tripoint(posx, posy, 0), tr->id);
    }
...
int iuse::throwable_extinguisher_act(player *, item *it, bool, const tripoint &pos)
{
    if( g->m.get_field( pos, fd_fire ) != nullptr ) {
        ...
        for (int x = -1; x <= 1; x++) {
            for (int y = -1; y <= 1; y++) {
                if ((g->m.move_cost(pos) != 0) && (x == 0 || y == 0)) {
                    g->m.adjust_field_strength(pos, fd_fire, 0 - rng(0, 1));
                }
            }
        }
        return 1;
...
int iuse::pipebomb_act(player *, item *it, bool t, const tripoint &pos)
{
    if (t) { // Simple timer effects
        ...
        g->sound(pos, 0, _("ssss...")); // Vol 0 = only heard if you hold it
    } else if (it->charges > 0) {
...
int iuse::granade_act(player *, item *it, bool t, const tripoint &pos)
{
    ...
    if (t) { // Simple timer effects
        g->sound(pos, 0, _("Merged!"));  // Vol 0 = only heard if you hold it
    } else if (it->charges > 0) {
            ...
            case 1:
                ...
                for (int i = -explosion_radius; i <= explosion_radius; i++) {
                    for (int j = -explosion_radius; j <= explosion_radius; j++) {
                        const tripoint p2(pos.x + i, pos.y + j, pos.z);
                        const int zid = g->mon_at(p2);
                        if (zid != -1 &&
    ...
    if (!p->has_item(it)) {
        ...
        for (int x = pos.x - 1; x <= pos.x + 1; x++) {
            for (int y = pos.y - 1; y <= pos.y + 1; y++) {
                g->m.add_field(tripoint(x, y, pos.z), fd_acid, 3);
            }
        }
    }
    return 0;
...
{
    if (t) { // Simple timer effects
        g->sound(pos, 0, _("Tick!")); // Vol 0 = only heard if you hold it
    } else if (it->charges > 0) {
        ...
        for (int i = -2; i <= 2; i++) {
            for (int j = -2; j <= 2; j++) {
                g->m.add_field(tripoint(pos.x + i, pos.y + j, pos.z), fd_incendiary, 3);
            }
        }
...
int iuse::mp3_on(player *p, item *it, bool t, const tripoint &)
{
    if (t) { // Normal use
...
int iuse::knife(player *p, item *it, bool t, const tripoint &)
{
    int choice = -1;
...
int iuse::torch_lit(player *p, item *it, bool t, const tripoint &)
{
    if (p->is_underwater()) {
...
int iuse::battletorch_lit(player *p, item *it, bool t, const tripoint &)
{
    if (p->is_underwater()) {
...
int iuse::talking_doll(player *p, item *it, bool, const tripoint &)
{
    ...
    g->ambient_sound(p->pos(), speech.volume, speech.text);
...
int iuse::bell(player *p, item *it, bool, const tripoint &)
{
    if (it->type->id == "cow_bell") {
        g->sound(p->pos(), 12, _("Clank! Clank!"));
        if (!p->is_deaf()) {
...
int iuse::einktabletpc(player *p, item *it, bool t, const tripoint &pos)
{
    if (t) {
        if (it->item_vars["EIPC_MUSIC_ON"] != "") {
            ...
            if (g->sound(pos, 8, "") && !p->has_effect("music")) {
...
int iuse::ehandcuffs(player *p, item *it, bool t, const tripoint &pos)
{
    if (t) {
        ...
        if (calendar::turn % 10 == 0) {
            g->sound(pos, 10, _("a police siren, whoop WHOOP."));
        }

        const int x = atoi(it->item_vars["HANDCUFFS_X"].c_str());
        const int y = atoi(it->item_vars["HANDCUFFS_Y"].c_str());
        const int z = atoi(it->item_vars["HANDCUFFS_Z"].c_str());

        if ((it->charges > it->type->maximum_charges() - 1000) && (x != pos.x || y != pos.y || z != pos.z)) {
            ...
            it->item_vars["HANDCUFFS_Z"] = string_format("%d", pos.z);
...
int iuse::remoteveh(player *p, item *it, bool t, const tripoint &)
{
    if (t) {
        if (it->charges == 0) {
            it->active = false;
