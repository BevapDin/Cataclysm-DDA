...
bool einkpc_download_memory_card(player *p, item *eink, item *mc)
{
    ...
    if (!mc->get_var( "MC_RECIPE" ).empty()) {
        ...
        for( auto &cur_recipe : recipe_dict ) {
            const int dif = cur_recipe.difficulty;

            if (science) {
                if( cur_recipe.cat != "CC_NONCRAFT" ) {
                    if (dif >= 3 && one_in(dif + 1)) {
                        candidates.push_back( &cur_recipe );
                    }
                }
            } else {
                if( cur_recipe.cat == "CC_FOOD" ) {
                    if (dif <= 3 && one_in(dif)) {
                        candidates.push_back( &cur_recipe );
                    }
                }
        ...
        if (candidates.size() > 0) {
            ...
            const std::string rident = r->ident().str();
    ...
    if (t) {
        ...
        if (ei_recipe == choice) {
            ...
            while (getline(f, s, ',')) {
                ...
                candidate_recipes.push_back( recipe_id( s ) );

                auto recipe = recipe_dict[recipe_id( s )];
                if( recipe ) {
            ...
            if (0 == rchoice) {
                ...
                auto recipe = recipe_dict[rec_id];
                if( recipe ) {
                    p->add_msg_if_player(m_info,
...
int iuse::multicooker(player *p, item *it, bool t, const tripoint &pos)
{
    if (t) {
        ...
        if (mc_start == choice) {
            ...
            for( auto &cur_recipe : recipe_dict ) {
                if( cur_recipe.cat == "CC_FOOD" && ( cur_recipe.subcat == "CSC_FOOD_MEAT" ||
