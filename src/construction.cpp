...
static bool player_can_build( player &p, const crafting_inventory_t &inv, construction const *con );
...
void move_ppoints_for_construction(const std::string &skillName, int difficulty, int &moves_left) {
    ...
    if(skill > 0 && skill > difficulty) {
        ...
        add_msg("%s-factor: %f", skillName.c_str(), skillfactor);
    }
}

void move_ppoints_for_construction(const construction &con, int &moves_left) {
    move_ppoints_for_construction("carpentry", con.difficulty, moves_left);
}

bool player_can_build(player &p, const crafting_inventory_t &pinv, const std::string &desc)
{
...
bool player_can_build(player &p, const crafting_inventory_t &pinv, construction const *con)
{
...
void check_constructions()
{
    for( auto const &a : constructions) {
        ...
        const_cast<construction*>(c)->requirements.auto_functions();
