...
void computer::activate_function(computer_action action)
{
    ...
    case COMPACT_TOLL:
        ...
        g->sound(g->u.pos(), 120, _("Bohm... Bohm... Bohm..."));
        break;
    ...
    case COMPACT_RELEASE:
        ...
        g->sound(g->u.pos(), 40, _("An alarm sounds!"));
    ...
    case COMPACT_MISS_LAUNCH: {
        ...
        g->explosion(tripoint(g->u.posx + 10, g->u.posx + 21, 0), 200, 0, true); //Only explode once. But make it large.
    ...
    case COMPACT_SRCF_SEAL:
        ...
        for (int x = 0; x < SEEX * MAPSIZE; x++) {
            for (int y = 0; y < SEEY * MAPSIZE; y++) {
                ...
                if (g->m.ter(x, y) == t_sewage_pump) {
                    g->m.make_rubble(x, y, f_rubble_rock, true);
                    g->explosion(tripoint(x, y, 0), 50, 0, true);
                }
            }
        }
...
void computer::activate_failure(computer_failure fail)
{
    ...
    case COMPFAIL_ALARM:
        ...
        g->sound(g->u.pos(), 60, _("An alarm sounds!"));
    ...
    case COMPFAIL_PUMP_EXPLODE:
        ...
        for (int x = 0; x < SEEX * MAPSIZE; x++) {
            for (int y = 0; y < SEEY * MAPSIZE; y++) {
                if (g->m.ter(x, y) == t_sewage_pump) {
                    g->m.make_rubble(x, y);
                    g->explosion(tripoint(x, y, 0), 10, 0, false);
                }
            }
        }
        break;
    ...
    case COMPFAIL_AMIGARA:
        ...
        g->u.add_effect("amigara", 20);
        g->explosion(tripoint(rng(0, SEEX * MAPSIZE), rng(0, SEEY * MAPSIZE), 0), 10, 10, false);
        g->explosion(tripoint(rng(0, SEEX * MAPSIZE), rng(0, SEEY * MAPSIZE), 0), 10, 10, false);
        break;
