...
void computer::activate_function(computer_action action)
{
    ...
    case COMPACT_OPEN:
        g->m.translate_radius(t_door_metal_locked, t_floor, 25.0, g->u.xpos(), g->u.ypos());
    ...
    case COMPACT_LOCK:
        g->m.translate_radius(t_door_metal_c, t_door_metal_locked, 8.0, g->u.xpos(), g->u.ypos());
        query_any(_("Lock enabled.  Press any key..."));
        break;

    case COMPACT_UNLOCK:
        g->m.translate_radius(t_door_metal_locked, t_door_metal_c, 8.0, g->u.xpos(), g->u.ypos());
    ...
    case COMPACT_TOLL:
        ...
        g->sound(g->u.xpos(), g->u.ypos(), 120, _("Bohm... Bohm... Bohm..."));
        break;
    ...
    case COMPACT_RELEASE:
        ...
        g->sound(g->u.xpos(), g->u.ypos(), 40, _("An alarm sounds!"));
        g->m.translate_radius(t_reinforced_glass_h, t_floor, 25.0, g->u.xpos(), g->u.ypos());
        g->m.translate_radius(t_reinforced_glass_v, t_floor, 25.0, g->u.xpos(), g->u.ypos());
    ...
    case COMPACT_CASCADE: {
        ...
        for (int i = g->u.xpos() - 10; i <= g->u.xpos() + 10; i++) {
            for (int j = g->u.ypos() - 10; j <= g->u.ypos() + 10; j++) {
        ...
        if (cascade_points.empty()) {
            g->resonance_cascade(g->u.xpos(), g->u.ypos());
        } else {
    ...
    case COMPACT_MISS_LAUNCH: {
        ...
        for(int i = g->u.xpos() + 8; i < g->u.xpos() + 15; i++) {
            for(int j = g->u.ypos() + 3; j < g->u.ypos() + 12; j++)
                if(!one_in(4)) {
                    g->m.add_field(i + rng(-2, 2), j + rng(-2, 2), fd_smoke, rng(1, 9));
                }
        }

        g->explosion(g->u.xpos() + 10, g->u.xpos() + 21, 200, 0, true); //Only explode once. But make it large.
...
of pureed bone & LSD."));
    ...
    case COMPACT_BLOOD_ANAL:
        for (int x = g->u.xpos() - 2; x <= g->u.xpos() + 2; x++) {
            for (int y = g->u.ypos() - 2; y <= g->u.ypos() + 2; y++) {
                ...
                if (g->m.ter(x, y) == t_floor_blue) {
                    ...
                    if (g->m.i_at(x, y).empty()) {
                        ...
                        if (g->m.i_at(x, y)[0].type->id == "black_box") {
                            ...
                            g->m.add_item_or_charges(g->u.xpos(), g->u.ypos(), transcript);
                        } else {
...
void computer::activate_failure(computer_failure fail)
{
    ...
    case COMPFAIL_ALARM:
        ...
        g->sound(g->u.xpos(), g->u.ypos(), 60, _("An alarm sounds!"));
    ...
    case COMPFAIL_MANHACKS: {
        ...
        for (int i = 0; i < num_robots; i++) {
            int mx, my, tries = 0;
            do {
                mx = rng(g->u.xpos() - 3, g->u.xpos() + 3);
                my = rng(g->u.ypos() - 3, g->u.ypos() + 3);
                tries++;
