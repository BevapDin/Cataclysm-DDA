...
long firestarter_actor::use( player &p, item &it, bool t, const tripoint &spos ) const
{
    ...
    const time_duration min_duration = std::min( duration_base,
                                       sqrt( 1 + duration_base / 1_moves ) * 1_moves );
    const time_duration duration = time_duration::from_moves( std::max( min_duration,
                                   duration_base * moves_modifier ) / light );
    ...
    p.assign_activity( activity_id( "ACT_START_FIRE" ), duration, -1, p.get_item_position( &it ),
                       it.tname() );
...
long ammobelt_actor::use( player &p, item &, bool, const tripoint & ) const
{
    ...
    if( opt ) {
        p.assign_activity( activity_id( "ACT_RELOAD" ), time_duration::from_moves( opt.moves() ),
                           opt.qty() );
