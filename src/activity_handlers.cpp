...
void activity_handlers::train_finish( player_activity *act, player *p )
{
    ...
    if( sk.is_valid() ) {
        ...
        catalua::mod_callback( *g->lua_engine_ptr, "on_player_skill_increased", p->getID(), skill_increase_source, sk.str(), new_skill_level );
        catalua::mod_callback( *g->lua_engine_ptr, "on_skill_increased" ); // legacy callback without arguments
        act->set_to_null();
        return;
    }
