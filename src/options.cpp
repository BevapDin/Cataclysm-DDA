...
void options_manager::addOption( Args &&... args )
{
    ...
    const std::string page = ptr->metadata().getPage();
    const auto pair = options.insert( std::make_pair( name, std::move( ptr ) ) );
    if( !pair.second ) {
...
void options_manager::Page::removeDoubleEmptyLines() {
    const auto empty = [&](const cata::optional<std::string> &v ) -> bool {
        return !v || get_options().get_option( *v ).metadata().is_hidden();
    };
...
{
    const int iWorldOptPage = std::find_if( pages_.begin(), pages_.end(), [&](const Page &p) { return &p == &world_default_page_; } ) - pages_.begin();
    ...
    while( true ) {
        ...
        for( int i = iStartPos;
            ...
            const OptionValueBase &current_opt = *cOPTIONS[*opt_name];
        ...
        OptionValueBase &current_opt = *cOPTIONS[opt_name];
...
#endif
        {
            fold_and_print( w_options_tooltip, point_zero, iMinScreenWidth - 2, c_white, "%s #%s",
                            current_opt.metadata().getTooltip(),
                            current_opt.metadata().getDefaultText() );
        }
