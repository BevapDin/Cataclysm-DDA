...
void event::actualize()
{
    ...
    case EVENT_SPAWN_WYRMS: {
        ...
        if (!one_in(25)) { // They just keep coming!
            event_manager::get_instance().add(EVENT_SPAWN_WYRMS, int(calendar::turn) + rng(15, 25));
        }
    } break;
...
// flood_buf is filled with correct tiles; now copy them back to g->m
   ...
   event_manager::get_instance().add(EVENT_TEMPLE_FLOOD, int(calendar::turn) + rng(2, 3));
  } break;
