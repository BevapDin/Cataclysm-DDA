...
void vehicle_stack::insert_at( std::list<item>::iterator index,
                               const item &newitem )
{
    vpart_reference( *myorigin, part_num ).add_item_at( index, newitem );
}
...
void vehicle::init_state( int init_veh_fuel, int init_veh_status )
{
        ...
        if( veh_status == 0 ) {
            ...
            if( roll < unhurt ) {
                if( roll <= broken ) {
                    ...
                    pt.set_hp( ( roll - broken ) / double( unhurt - broken ) * vp.info().durability );
                }
            } else {
                pt.set_hp( vp.info().durability );
            }
    ...
    if( destroyTires && !wheelcache.empty() ) {
        ...
        while( valid_wheel_config( false ) && tries < 100 ) {
            ...
            parts[random_entry( wheelcache )].set_hp( 0 );
            tries++;
        }
    }
...
{
    return has_value() ? value().part_with_feature( f ) : cata::nullopt;
}

cata::optional<vpart_reference> optional_vpart_position::part_with_feature( const vpart_bitflags f ) const
{
    return has_value() ? value().part_with_feature( f ) : cata::nullopt;
}

cata::optional<vpart_reference> optional_vpart_position::part_with_feature_including_broken( const std::string &f ) const
{
    return has_value() ? value().part_with_feature_including_broken( f ) : cata::nullopt;
}

cata::optional<vpart_reference> optional_vpart_position::part_with_feature_including_broken( const vpart_bitflags f ) const
{
    return has_value() ? value().part_with_feature_including_broken( f ) : cata::nullopt;
}
...
{
    // We take the last part, since we close the outermost thing first (curtains), and then the innermost thing (door)
    cata::optional<vpart_reference> result;
    for( const vpart_reference vp : parts_here() ) {
        if( vp.has_feature( VPFLAG_OPENABLE )
            && vp.part().is_available()
            && vp.part().open == 1
            && ( !outside || !vp.has_feature( "OPENCLOSE_INSIDE" ) ) ) {
            result.emplace( vp );
        }
    }
    return result;
...
cata::optional<vpart_reference> vehicle::next_part_to_open( const bool outside ) const
{
    // We want forwards, since we open the innermost thing first (curtains), and then the innermost thing (door)
    for(  const vpart_reference vp : parts_here() ) {
        if( vp.has_feature( VPFLAG_OPENABLE ) && vp.part().is_available() && vp.part().open == 0 &&
            ( !outside || !vp.has_feature( "OPENCLOSE_INSIDE" ) ) ) {
            return vp;
        }
    }
...
{
    return vehicle_part_with_feature_range<std::string>( const_cast<vehicle &>( *this ), std::move( feature ), true, false );
}

vehicle_part_with_feature_range<vpart_bitflags> vehicle::get_parts( const vpart_bitflags feature ) const
{
    return vehicle_part_with_feature_range<vpart_bitflags>( const_cast<vehicle &>( *this ), feature, true, false );
}

vehicle_part_with_feature_range<std::string> vehicle::get_parts_including_broken( std::string feature ) const
{
    return vehicle_part_with_feature_range<std::string>( const_cast<vehicle &>( *this ), std::move( feature ), false, false );
}
...
bool vpart_reference::add_item( const item &obj ) const
{
    ...
    return add_item_at( part().items.end(), itm );
}

bool vpart_reference::add_item_at( const std::list<item>::iterator index, item itm ) const
{
    if( itm.is_bucket_nonempty() ) {
        for( auto &elem : itm.contents ) {
            g->m.add_item_or_charges( position(), elem );
        }

        itm.contents.clear();
    }

    const auto new_pos = part().items.insert( index, itm );
    if( itm.needs_processing() ) {
        vehicle().active_items.add( new_pos, mount() );
    }
...
void vehicle::refresh_insides()
{
    ...
    for( const vpart_reference vp : get_parts() ) {
        ...
        vp.part().inside = true; // inside if not otherwise
...
vehicle_part_with_feature_range<std::string> vehicle::get_parts( std::string feature ) const
{
    return vehicle_part_with_feature_range<std::string>( const_cast<vehicle &>( *this ), std::move( feature ), true );
}

vehicle_part_with_feature_range<vpart_bitflags> vehicle::get_parts( const vpart_bitflags feature ) const
{
    return vehicle_part_with_feature_range<vpart_bitflags>( const_cast<vehicle &>( *this ), feature, true );
}
...
{
    return this->vehicle().parts[part_index()];
}

bool vpart_reference::has_feature( const std::string &feature ) const
{
    return info().has_flag( feature );
}

bool vpart_reference::has_feature( const vpart_bitflags feature ) const
{
    return info().has_flag( feature );
}
