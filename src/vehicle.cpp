...
int vehicle::total_power (bool fueled)
{
    ...
    for (size_t e = 0; e < engines.size(); e++) {
        int p = engines[e];
        if (is_engine_on(e) && parts[p].active() && (fuel_left (part_info(p).fuel_type) || !fueled ||
...
void vehicle::power_parts ()//TODO: more categories of powered part!
{
    ...
    if(engine_on) {
        ...
        for( size_t e = 0; e < engines.size(); ++e ) {
            if((is_engine_type_on(e, fuel_type_gasoline) && parts[engines[e]].active()) || 
...
void vehicle::thrust (int thd) {
    ...
    if (thrusting && rng(1, accel) <= vel_inc ) {
        ...
        for (size_t e = 0; e < engines.size(); e++){
            size_t p = engines[e];
            if (is_engine_on(e) && parts[p].active() && fuel_left(part_info(p).fuel_type) && 
                rng (1, 100) < strn ) {
                int dmg = rng (strn * 2, strn * 4);
                damage_direct (p, dmg, 0);
                if(one_in(2)) {
