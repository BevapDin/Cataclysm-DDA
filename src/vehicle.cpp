...
struct part_type_comparator {
    ...
    bool operator()(int a, int b) const {
        ...
        if(veh->parts[a].mount.x != veh->parts[b].mount.x) {
            return veh->parts[a].mount.x < veh->parts[b].mount.x;
        }
        if(veh->parts[a].mount.y != veh->parts[b].mount.y) {
            return veh->parts[a].mount.y < veh->parts[b].mount.y;
        }
        return a < b;
    }
};
...
void vehicle::use_controls()
{
   ...
   if (has_overhead_lights) {
    ...
    if (engines.size() > 1) {
        menu.addentry( x_control_engines, true, 'E', _("Control engines") );
    }

    if (lights.size() > 1) {
        menu.addentry( control_lights, true, 'L', _("Control lights") );
    }
...
bool vehicle::toogle_active_menu(const std::vector<int> &parts_to_toggle, const std::string &title) {
    ...
    for(size_t i = 0; i < parts_to_toggle.size(); i++) {
        ...
        buffer << " " << part_info(p).name << " at " << part.mount.x << "," << part.mount.y;
...
int vehicle::install_part( int dx, int dy, const vehicle_part &new_part )
{
    ...
    debugmsg("Did not found new installed vpart %s at %d,%d", new_part.get_id().c_str(), dx, dy);
    return parts.size() - 1;
}
...
std::vector<int> vehicle::parts_at_relative (const int dx, const int dy, bool const use_cache) const
{
    ...
    while(u != l) {
        ...
        if(parts[pivo].mount.x < dx) {
            l = pivo + 1;
        } else if(parts[pivo].mount.x > dx) {
            u = pivo;
        } else if(parts[pivo].mount.y < dy) {
            l = pivo + 1;
        } else if(parts[pivo].mount.y > dy) {
            u = pivo;
        } else {
            // Found matching part, stop here
            assert(parts[pivo].mount.x == dx && parts[pivo].mount.y == dy);
            u = pivo;
            break;
        }
    }
    ...
    while(u > 0) {
        ...
        if(parts[u - 1].mount.x == dx && parts[u - 1].mount.y == dy) {
            u--;
        } else {
    ...
    for( ; (size_t)u < parts.size() && parts[u].mount.x == dx && parts[u].mount.y == dy; u++) {
        res.push_back(u);
    }
    return res;
}
...
int vehicle::part_with_feature (int part, const vpart_bitflags flag, const bool unbroken) const {
    const int dx = parts[part].mount.x;
    const int dy = parts[part].mount.y;
    while(part > 0) {
        if(parts[part - 1].mount.x == dx && parts[part - 1].mount.y == dy) {
            part--;
        } else {
            break;
        }
    ...
    for (size_t i = part; i < parts.size(); i++) {
        if (parts[i].mount.x == dx && parts[i].mount.y == dy) {
...
void vpart_range(vehicle *veh, int &part_begin, int &part_end, int dx, int dy) {
    ...
    while(part_begin > 0) {
        ...
        if(vp.mount.x == dx && vp.mount.y == dy) {
            part_begin--;
        } else {
            break;
        }
...
void vehicle::noise_and_smoke( double load, double time )
{
    ...
    for( size_t e = 0; e < engines.size(); e++ ) {
        int p = engines[e];
        if( is_engine_on(e) && parts[p].active() &&
...
void vehicle::idle(bool on_map) {
    ...
    if (engine_on && total_power() > 0) {
        for (size_t e = 0; e < engines.size(); e++){
            size_t p = engines[e];
            if (fuel_left(part_info(p).fuel_type) && is_engine_on(e) && parts[p].active()) {
...
bool vehicle::add_item(int p, item itm)
{
    ...
    for( auto & p : parts ) {
        if( p.info().has_flag( VPFLAG_CARGO ) ) {
            pptrs.insert( &p );
        }
    }
    for( auto & pp : pptrs ) {
...
bool vehicle::is_single_tile() const {
    ...
    for(size_t i = 0; i < parts.size(); i++) {
        if(parts[i].mount.x != 0 || parts[i].mount.y != 0) {
            return false;
        }
    }
    return true;
...
bool vehicle::tow_to(vehicle *other, int other_part, player *p) {
    ...
    const int px = other->parts[other_part].mount.x;
    const int py = other->parts[other_part].mount.y;
...
void vehicle::untow(int part, player *p) {
    ...
    auto items = get_items( part );
    assert(!items.empty());
    const int mx = parts[part].mount.x;
    const int my = parts[part].mount.y;
    const int gx = global_x() + parts[part].precalc[0].x;
    const int gy = global_y() + parts[part].precalc[0].y;
    // Recover rope and name of the towed vehicle
    item rope = items.front();
    items.erase(items.begin());
    const std::string new_veh_name = rope.get_var( "dfkjghdfkg" );
    rope.erase_var( "dfkjghdfkg" );
    p->add_or_drop(rope, g);
    
    // Remove all the parts of the towed vehicle
    std::vector<vehicle_part> tmp_parts;
    for(size_t i = 0; i < parts.size(); i++) {
        if(parts[i].mount.x == mx && parts[i].mount.y == my) {
            parts[i].mount.x = 0;
            parts[i].mount.y = 0;
...
//    sort_parts();
    ...
    vehicle *new_veh = g->m.add_vehicle( vproto_id( "custom" ), gx, gy, 270, 0, 0);
    if(new_veh == 0) {
...
void vehicle::sort_parts() {
    active_items.clear();
    std::vector<vehicle_part> ptmp;
    while( !parts.empty() ) {
        const point &mp = parts.front().mount;
        size_t j = 0;
        for( ; j < ptmp.size(); j++ ) {
            if(mp.x < ptmp[j].mount.x) {
                break;
            } else if(mp.x > ptmp[j].mount.x) {
                continue;
            }
            if(mp.y < ptmp[j].mount.y) {
                break;
            } else if(mp.y > ptmp[j].mount.y) {
                continue;
            }
            continue;
        }
        assert(j <= ptmp.size());
        ptmp.insert( ptmp.begin() + j, std::move( parts.front() ) );
