...
int vehicle::basic_consumption(const ammotype &ftype) const
{
    int fcon = 0;
    for( size_t e = 0; e < engines.size(); ++e ) {
        if(is_engine_type_on(e, ftype) && parts[engines[e]].active()) {
...
void vehicle::power_parts (tripoint sm_loc)//TODO: more categories of powered part!
{
    ...
    if(engine_on) {
        ...
        int plasma_epower = 0;
        for( size_t e = 0; e < engines.size(); ++e ) {
            if(is_engine_type_on(e, fuel_type_plasma) && parts[engines[e]].active()) {
...
void vehicle::sort_parts() {
    ...
    for(size_t i = 0; i < parts.size(); i++) {
        ...
        for(size_t j = 0; j < parts.size(); j++) {
            ...
            if(parts[p].mount.x > parts[j].mount.x) {
                p = j;
                continue;
            }
            if(parts[p].mount.x < parts[j].mount.x) {
            ...
            if(parts[p].mount.y > parts[j].mount.y) {
                p = j;
                continue;
            }
        }
