...
void Item_factory::check_itype_definitions() const
{
    ...
    for (std::map<Item_tag, itype *>::const_iterator it = m_templates.begin(); it != m_templates.end();
        ...
        if( type->gun_slot ) {
            check_ammo_type( msg, type->gun_slot->ammo );
            if( type->gun_slot->skill_used == nullptr ) {
...
template<>
void Item_factory::load_slot( std::unique_ptr<islot_gun> &slotptr, JsonObject &jo )
{
    // TODO: more generic. those two lines are identical for all slot types
    slotptr.reset( new islot_gun() );
    auto &slot = *slotptr;
...
void Item_factory::load_gun(JsonObject &jo)
{
    ...
    load_slot( new_item_template->gun_slot, jo );
    load_basic_info( jo, new_item_template.get() );
    new_item_template.release();
}

template<>
void Item_factory::load_slot( std::unique_ptr<islot_armor> &slotptr, JsonObject &jo )
{
    // TODO: more generic. those two lines are identical for all slot types
    slotptr.reset( new islot_armor() );
    auto &slot = *slotptr;
...
template<>
void Item_factory::load_slot( std::unique_ptr<islot_variable_bigness> &slotptr, JsonObject &jo )
{
    // TODO: more generic. those two lines are identical for all slot types
    slotptr.reset( new islot_variable_bigness() );
    auto &slot = *slotptr;
...
{
    std::unique_ptr<itype> new_item_template( new itype() );
    load_slot( new_item_template->variable_bigness_slot, jo );
    load_basic_info( jo, new_item_template.get() );
    new_item_template.release();
}
