...
// First, if we're wielding a gun, get some ammo for it
 if (me->weapon.is_gun()) {
  tmp = default_ammo(me->weapon.type->gun_slot->ammo);
  if (tmp == "" || tmp == "UPS"){
...
void npc::starting_weapon(npc_class type)
{
    ...
    if (weapon.is_gun())
    {
        const std::string tmp = default_ammo( weapon.type->gun_slot->ammo );
        if( tmp != "" ) {
            weapon.charges = weapon.type->gun_slot->clip;
...
void npc::decide_needs()
{
    ...
    for (int i = 0; i < num_needs; i++)
        needrank[i] = 20;
    if (weapon.is_gun()) {
        needrank[need_ammo] = 5 * has_ammo(weapon.type->gun_slot->ammo).size();
    }
...
int npc::value(const item &it)
{
 ...
 if (it.is_ammo()) {
  ...
  if (weapon.is_gun()) {
   if (ammo->type == weapon.type->gun_slot->ammo)
    ret += 14;
  }
 ...
 if (it.is_book()) {
  auto book = it.type->book_slot.get();
