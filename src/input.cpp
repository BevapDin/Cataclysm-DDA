...
void input_manager::load( const std::string &file_name, bool is_user_preferences )
{
    ...
    while( !jsin.end_array() ) {
        ...
        while( bindings.has_more() ) {
            ...
            events.emplace_back( keybinding );
        }
...
std::string input_manager::get_keyname( long ch, input_event_t inp_type ) const
{
...
std::string input_manager::get_keyname_portable( long ch, input_event_t inp_type ) const
{
    if( inp_type == CATA_INPUT_KEYBOARD ) {
        const t_key_to_name_map::const_iterator a = keycode_to_keyname.find( ch );
        if( a != keycode_to_keyname.end() ) {
...
const std::string &input_context::handle_input()
{
    ...
    while( 1 ) {
        next_action = inp_mngr.get_input_event( category );
    ...
    while( true ) {
        ...
        if( action == "ADD_LOCAL" || raw_input_char == '+' ) {
            ...
            if( status == s_remove && ( !get_option<bool>( "QUERY_KEYBIND_REMOVAL" ) ||
                ...
                const input_event new_event = popup_getkey( _( "New key for %s:" ), name.c_str() );
...
#endif
    while( true ) {
        switch( inp_mngr.get_input_event( "WAIT_FOR_ANY_KEY" ).type ) {
            case CATA_INPUT_KEYBOARD:
                return;
...
class macro_controller
{
    ...
    public:
        ...
        bool is_toogle_record( const input_event &event ) const {
            return event.type == CATA_INPUT_KEYBOARD && event.get_first_input() == KEY_F( 1 );
        }
        ...
        void save_impl() {
            ...
            assure_dir_exist(FILENAMES["macrodir"]);
            const std::string path = FILENAMES["macrodir"] + name + ".json";
            if(file_exist(path)) {
        ...
        void load_impl() {
            ...
            for( std::string path : get_files_from_path( ".json", FILENAMES["macrodir"], false, true ) ) {
        ...
        bool on_input( const std::string &category, const input_event &event ) {
            if( state == state_type::DISABLED) {
                ...
                if( state == state_type::RECORDING ) {
                    recording.push_back( { event, category } );
                }
                return true;
            }
        }
        bool is_playing( const std::string &category, input_event &event ) {
            ...
            if( next.category != category ) {
                state = state_type::NONE;
                popup( _( "Category mismatch in macro: %s vs %s" ), category.c_str(), next.category.c_str() );
                return false;
            }
            event = next.event;
