...
bool game::cleanup_at_end()
{
    ...
    if( uquit == QUIT_DIED || uquit == QUIT_SUICIDE ) {
        ...
        int minutes = to_minutes<int>( time_duration::from_turns( turns ) % 1_hours );
        int hours = to_hours<int>( time_duration::from_turns( turns ) % 1_days );
        int days = to_days<int>( time_duration::from_turns( turns ) );
...
void game::debug()
{
    ...
    switch( action ) {
        ...
        case 25:
            ...
            break;
        case 26: {
            auto set_turn = [&]( const time_duration & factor, const cata::optional<time_duration> larger,
            ...
            do {
                ...
                smenu.reset();
                smenu.addentry( 0, true, 'y', "%s: %d", _( "year" ),
                                static_cast<int>( calendar::turn / calendar::year_length() ) );
                ...
                switch( smenu.ret ) {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        set_turn( 1_turns, 1_days, string_format( _( "Set turn to? (One day is %i turns)" ),
                                  to_turns<int>( 1_day ) ) );
                        break;
                    default:
                        break;
...
void game::reload( item_location &loc, bool prompt )
{
    ...
    if( opt ) {
        u.assign_activity( activity_id( "ACT_RELOAD" ), time_duration::from_moves( opt.moves() ),
                           opt.qty() );
        if( use_loc ) {
...
void game::reload()
{
    ...
    if( !u.is_armed() || !u.can_reload( u.weapon ) || u.weapon.has_flag( "RELOAD_AND_SHOOT" ) ) {
        ...
        if( veh && ( turret = veh->turret_query( u.pos() ) ) && turret.can_reload() ) {
            ...
            if( opt ) {
                g->u.assign_activity( activity_id( "ACT_RELOAD" ), time_duration::from_moves( opt.moves() ),
                                      opt.qty() );
        ...
        if( get_option<bool>( "AUTO_FEATURES" ) && mostseen == 0 && get_option<bool>( "AUTO_MINING" ) &&
            ...
            if( u.weapon.has_flag( "POWERED" ) ) {
                if( u.weapon.ammo_sufficient() ) {
                    ...
                    u.assign_activity( activity_id( "ACT_JACKHAMMER" ), 30_minutes, -1,
