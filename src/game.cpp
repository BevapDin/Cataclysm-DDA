...
void game::create_starting_npcs()
{
    ...
    tmp->add_new_mission( mission::reserve_random( ORIGIN_OPENER_NPC, tmp->global_omt_location(), creature_reference( *tmp ) ) );
}
    ...
    if( u.getID() == 0 || u.getID() == -1 ) {
        ...
        if( u.in_vehicle ) {
            ...
            if( veh != nullptr ) {
                ...
                if( vpart >= 0 ) {
                    veh->parts[vpart].passenger = creature_reference( u );
                }
            }
        }
    }
...
void game::spawn_mon(int /*shiftx*/, int /*shifty*/)
{
    ...
    if( x_in_y( density, 100 ) ) {
        ...
        tmp->add_new_mission( mission::reserve_random(ORIGIN_ANY_NPC, tmp->global_omt_location(), creature_reference( *tmp ) ) );
