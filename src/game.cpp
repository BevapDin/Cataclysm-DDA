...
void game::catch_a_monster(std::vector<monster*> &catchables, const tripoint &pos, player *p, int catch_duration) // catching function
{
    ...
    //todo change catch_duration to time_duration
    m.add_item_or_charges( pos, item::make_corpse( fish->type->id, calendar::turn + rng( 0_turns, time_duration::from_turns( catch_duration ) ) ) );
