        ...
        for( auto v : m.get_vehicles() ) {
            ...
            if( name.find( search ) != std::string::npos ) {
                for( const vpart_reference vp : v.v->get_parts_including_broken( VPFLAG_CONTROLS ) ) {
                    const tripoint pos = vp.position();
                    u.setpos( pos );
...
void game::control_vehicle()
{
    ...
    } else if( veh && veh->avail_part_with_feature( veh_part, "CONTROLS", true ) >= 0 && u.in_vehicle ) {
...
bool game::plfire_check( const targeting_data &args )
{
    ...
    if( args.mode != TARGET_MODE_TURRET_MANUAL ) {
        ...
        if( gun->has_flag( "MOUNTED_GUN" ) ) {
            const bool v_mountable = static_cast<bool>( m.veh_at( u.pos() ).part_with_feature( "MOUNTABLE", true ) );
...
bool game::prompt_dangerous_tile( const tripoint &dest_loc ) const
{
    ...
    if( !u.is_blind() ) {
        ...
        const bool boardable = static_cast<bool>( m.veh_at( dest_loc ).part_with_feature( "BOARDABLE", true ) );
