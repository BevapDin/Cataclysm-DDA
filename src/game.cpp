    ...
    do {
        ...
        if (action == "MOUSE_MOVE") {
            ...
            if (!ctxt.get_coordinates(w_terrain, mx, my)) {
                ...
                liveview.show(tripoint(mx, my, u.view_offset.z));
            }
        }
...
void game::rcdrive(int dx, int dy)
{
    ...
    if( m.move_cost(dp) == 0 || !m.can_put_items(dp) ||
        m.has_furn(dp) ) {
        sounds::sound(dp, 7, _("sound of a collision with an obstacle."));
        return;
    } else if( m.add_item_or_charges(dp, *rc_car ) ) {
        //~ Sound of moving a remote controlled car
        sounds::sound(d, 6, _("zzz..."));
        u.moves -= 50;
        m.i_rem( d, rc_car );
...
void game::draw_ter( const tripoint &center, bool looking )
{
    ...
    if (!destination_preview.empty()) {
        ...
        point final_destination = destination_preview.back();
        point center = point(u.posx() + u.view_offset.x, u.posy() + u.view_offset.y);
...
void game::explosion(const tripoint &pos, int power, int shrapnel, bool fire, bool blast)
{
    ...
    if (power >= 30) {
        sounds::sound(pos, noise, _("a huge explosion!"));
    } else if (power >= 4) {
        sounds::sound(pos, noise, _("an explosion!"));
    } else {
        sounds::sound(pos, 3, _("a loud pop!"));
    }
    if (blast) {
...
void game::shockwave(const tripoint &pos, int radius, int force, int stun, int dam_mult, bool ignore_player)
{
    ...
    sounds::sound(pos, force * force * dam_mult / 2, _("Crack!"));
    for (size_t i = 0; i < num_zombies(); i++) {
        monster &critter = critter_tracker.find(i);
...
void game::emp_blast(const tripoint &p)
{
    ...
    if (m.ter(p) == t_card_science || m.ter(p) == t_card_military) {
        ...
        if (rn > 80) {
            ...
            for (int i = -3; i <= 3; i++) {
                for (int j = -3; j <= 3; j++) {
                    const tripoint p2( p.x + i, p.y + j, p.z );
                    if (m.ter(p2) == t_door_metal_locked) {
                        m.ter_set(p2, t_floor);
                    }
                }
            }
        }
...
void game::print_trap_info(const tripoint &p, WINDOW *w_look, const int column, int &line)
{
    ...
    if (traplist[trapid]->can_see(u, p)) {
...
{
    const auto old_view_offset = u.view_offset;
    u.view_offset = tripoint( 0, 0, 0 );
...
tripoint game::look_around(WINDOW *w_info, const point pairCoordsFirst)
{
    ...
    int lx = u.posx() + u.view_offset.x;
    int ly = u.posy() + u.view_offset.y;
    int lz = u.view_offset.z;
    ...
    do {
        ...
        if (!handle_mouseview(ctxt, action)) {
            ...
            if (action == "TOGGLE_FAST_SCROLL") {
                ...
                draw_ter( tripoint( lx, ly, lz ), true );
                draw_footsteps( w_terrain, {POSX - lx, POSY - ly} );
            }
        }
...
void game::draw_trail_to_square(int x, int y, bool bDrawX)
{
    ...
    if (bDrawX) {
        if (vPoint.empty()) {
            ...
            mvwputch(w_terrain, POSY + (vPoint[vPoint.size() - 1].y - (u.posy() + u.view_offset.y)),
                     POSX + (vPoint[vPoint.size() - 1].x - (u.posx() + u.view_offset.x)),
                     c_white, 'X');
        }
    }
    ...
    if (toSwimmable && toDeepWater && !toBoat) { // Dive into water!
        ...
        if (!u.has_artifact_with(AEP_STEALTH) && !u.has_trait("LEG_TENTACLES") &&
            ...
            if (u.has_trait("LIGHTSTEP") || u.is_wearing("rm13_armor_on")) {
                sounds::sound(target, 2, "");    // Sound of footsteps may awaken nearby monsters
            } else if (u.has_trait("CLUMSY")) {
                sounds::sound(target, 10, "");
            } else if (u.has_bionic("bio_ankles")) {
                sounds::sound(target, 12, "");
            } else {
                sounds::sound(target, 6, "");
            }
        }
 ...
 */
bool game::find_vertical_movement_destination(tripoint &stair, int movez, bool &danger_lava) {
    ...
    for (ts.x = omtile_align_start.x; ts.x < omtile_align_end.x; ts.x++) {
        for (ts.y = omtile_align_start.y; ts.y < omtile_align_end.y; ts.y++) {
            ...
            if (rl_dist(u.pos(), ts) <= 3 && (m.ter(ts) == t_lava)) {
                danger_lava = true;
            }
        }
    }
...
void game::vertical_move(int movez, bool force)
{
    ...
    if (force) {
        ...
        if(!find_vertical_movement_destination(stair, movez, danger_lava)) { // No stairs found!
    ...
    if (m.tr_at(u.pos()) != tr_null) { // We stepped on a trap!
        trap *tr = traplist[m.tr_at(u.pos())];
        if (force || !u.avoid_trap(tr, u.pos())) {
            tr->trigger(&u, u.pos());
        }
    }
...
void game::update_map(int &x, int &y, int z)
{
    ...
    if (calendar::turn >= nextspawn) {
        spawn_mon(tripoint(shiftx, shifty, shiftz));
    }
