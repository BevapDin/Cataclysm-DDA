...
void game::rod_fish(int sSkillLevel, int fishChance) // fish-with-rod fish catching function
{
    if (sSkillLevel > fishChance) {
        ...
        if (fishables.size() < 1){
            ...
            m.add_item_or_charges(u.pos(), fish);
                ...
                */
                ...
                for( auto &elem : wPrint.vdrops ) {
                    m.drawsq( w_terrain, u, elem.first + offset_x, elem.second + offset_y, false,
                              true, tripoint( u.posx + u.view_offset_x, u.posy + u.view_offset_y, u.view_offset_z ),
                              false, false );
                }
                            ...
                            for( size_t i = 0; i < elem.getText().length(); ++i ) {
                                if( u_see( elem.getPosX() + i, elem.getPosY() ) ) {
                                    m.drawsq( w_terrain, u, elem.getPosX() + i, elem.getPosY(),
                                              false, true,
                                              tripoint( u.posx + u.view_offset_x,
...
void game::draw_critter(const Creature &critter, const tripoint &center)
{
    ...
    const bool can_see = m.pl_sees( u.pos(), critter.pos(),
    ...
    for( auto &c : u.get_visible_creatures( SEEX * MAPSIZE ) ) {
        ...
        if( m != nullptr ) {
            ...
            if (MATT_ATTACK == matt || MATT_FOLLOW == matt) {
                int j;
                if (index < 8 && sees_u(critter.pos(), j)) {
...
bool pet_menu(monster *z)
{
    ...
    if (give_items == choice) {
        ...
        bool success = g->make_drop_activity( ACT_STASH, point( z->xpos(), z->ypos() ) );
        if( success ) {
...
void game::peek()
{
    tripoint pos( u.posx, u.posy, u.view_offset_z );
    // TODO: Z
...
void game::peek(tripoint pos)
{
    ...
    prevx = u.posx;
    prevy = u.posy;
    prevz = u.view_offset_z;
    u.posx = pos.x;
    u.posy = pos.y;
    u.view_offset_z = pos.z;
    look_around();
    u.posx = prevx;
    u.posy = prevy;
    u.view_offset_z = prevz;
    draw_ter();
}
////////////////////////////////////////////////////////////////////////////////////////////
...
{
    if (pos == INT_MIN) {
        make_drop_activity( ACT_DROP, point( u.xpos(), u.ypos() ) );
...
void game::complete_butcher(int index)
{
    ...
    if ((int)m.i_at(u.pos()).size() <= index || m.i_at(u.pos())[index].corpse == NULL ||
        m.i_at(u.pos())[index].typeId() != "corpse") {
        add_msg(m_info, _("There's no corpse to butcher!"));
        return;
    }
    mtype *corpse = m.i_at(u.pos())[index].corpse;
    std::vector<item> contents = m.i_at(u.pos())[index].contents;
    int age = m.i_at(u.pos())[index].bday;
    ...
    for( auto &content : contents ) {
        if ((skill_shift + 10) * 5 > rng(0, 100)) {
            add_msg( m_good, _( "You discover a %s in the %s!" ), content.tname().c_str(),
                     corpse->nname().c_str() );
            m.add_item_or_charges( u.pos(), content );
        } else if( content.is_bionic() ) {
            m.spawn_item(u.pos(), "burnt_out_bionic", 1, 0, age);
        }
    }
    ...
    if (pieces <= 0) {
        ...
        while ( pieces > 0 ) {
            pieces--;
            m.add_item_or_charges(u.pos(), tmpitem);
        }
    }
}
        ...
        while (!it.contents.empty()) {
            ...
            if (content.made_of(LIQUID)) {
                ...
                if (u.can_pickVolume(content.volume()) &&
                    ...
                    m.add_item_or_charges(u.pos(), content, 1);
                }
            }
            it.contents.erase(it.contents.begin());
        }
    ...
    if (newam.made_of(LIQUID)) {
        ...
        if( !u.can_pickVolume( newam.volume() ) ) {
            add_msg( _( "There's no room in your inventory for the %s, so you drop it." ),
                    newam.tname().c_str() );
            m.add_item_or_charges( u.pos(), newam );
    ...
    while (range > 0) {
        ...
        if( thru ) {
            if( p != nullptr ) {
                ...
                zz->setpos(tripoint(x, y, zz->zpos()));
            }
        } else {
            break;
        }
