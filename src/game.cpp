...
void game::update_scent()
{
    ...
    if (u.posx == player_last_position.x && u.posy == player_last_position.y) {
        if (player_last_moved + 1000 < (int) calendar::turn) {
            return;
        }
    } else {
...
int game::list_items(const int iLastState)
{
    ...
    ctxt.register_action("EXCLUDE_CLOTHING");
                         ...
                         item *cont)
{
    ...
    if( cont == NULL && last_item_pos != INT_MIN ) {
        ...
        if( cont->get_remaining_capacity_for_liquid( liquid ) <= 0 ) {
...
void game::complete_butcher(int index)
{
    ...
    if( u.activity.values.size() > 0 && u.activity.values[0] == 1 ) {
        ...
        for( size_t i = 0; i < itms.size(); ++i ) {
            ...
            int time_to_cut = 0;
            switch (itms[i].get_mtype()->size) { // Time in turns to cut up te corpse
            case MS_TINY:
                time_to_cut = 2;
                break;
            case MS_SMALL:
    ...
    for (int x = -REVEAL_RADIUS; x <= REVEAL_RADIUS; x++) {
        for (int y = -REVEAL_RADIUS; y <= REVEAL_RADIUS; y++) {
            ...
            if (!!OPTIONS["AUTO_NOTES"]) {
                if (movez == +1 && otermap[ter].known_up && !otermap[ter2].known_down && !overmap_buffer.is_explored(cursx, cursy, z_coord)) {
                    overmap_buffer.set_seen(cursx, cursy, z_coord, true);
                    overmap_buffer.add_note(cursx, cursy, z_coord, _(">:W;AUTO: goes down"));
                }
                if (movez == -1 && otermap[ter].known_down && !otermap[ter2].known_up && !overmap_buffer.is_explored(cursx, cursy, z_coord)) {
                    overmap_buffer.set_seen(cursx, cursy, z_coord, true);
