...
void game::update_scent()
{
 ...
 for (int x = u.posx - SCENT_RADIUS -1; x <= u.posx + SCENT_RADIUS + 1; x++) {
  for (int y = u.posy - SCENT_RADIUS; y <= u.posy + SCENT_RADIUS; y++) {
   ...
   squares_used_y[x][y] =(m.move_cost_ter_furn(x,y-1) > 0   || m.is_bashable(x, y-1)) +
                                   (m.move_cost_ter_furn(x,y)   > 0   || m.is_bashable(x, y))   +
                                   (m.move_cost_ter_furn(x,y+1) > 0   || m.is_bashable(x, y+1)) ;
  }
 }
