...
void monster::try_reproduce()
{
    ...
    while( true ) {
        ...
        if( season_spawn ) {
            season_match = false;
            for( auto &elem : type->baby_flags ) {
                if( ( season_of_year( calendar::time_of_cataclysm + time_duration::from_days(
                                          baby_timer ) ) == SUMMER && elem == "SUMMER" ) ||
                    ( season_of_year( calendar::time_of_cataclysm + time_duration::from_days(
                                          baby_timer ) ) == WINTER && elem == "WINTER" ) ||
                    ( season_of_year( calendar::time_of_cataclysm + time_duration::from_days(
                                          baby_timer ) ) == SPRING && elem == "SPRING" ) ||
                    ( season_of_year( calendar::time_of_cataclysm + time_duration::from_days(
                                          baby_timer ) ) == AUTUMN && elem == "AUTUMN" ) ) {
                    season_match = true;
                }
            }
        }
        ...
        if( season_match && female && one_in( chance ) ) {
            ...
            if( type->baby_monster ) {
                ...
                g->m.add_item_or_charges( pos(), item( type->baby_egg,
                                                       calendar::time_of_cataclysm + time_duration::from_days( baby_timer ), spawn_cnt ), true );
            }
        }
...
void monster::try_biosignature()
{
    ...
    while( true ) {
        ...
        g->m.add_item_or_charges( pos(), item( type->biosig_item,
                                               calendar::time_of_cataclysm + time_duration::from_days( biosig_timer ), 1 ), true );
