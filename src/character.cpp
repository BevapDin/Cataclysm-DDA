...
float Character::healing_rate_medicine( float at_rest_quality, const body_part bp ) const
{
    ...
    if( !e_bandaged.is_null() ) {
        bandaged_rate += static_cast<float>( e_bandaged.get_amount( "HEAL_RATE" ) ) / to_turns<int>( 1_days );
        if( bp == bp_head ) {
    ...
    if( !e_disinfected.is_null() ) {
        disinfected_rate += static_cast<float>( e_disinfected.get_amount( "HEAL_RATE" ) ) / to_turns<int>( 1_days );
        if( bp == bp_head ) {
