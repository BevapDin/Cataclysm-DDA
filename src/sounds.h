...
namespace sounds {
    ...
    void sound(tripoint p, int vol, std::string description, bool ambient = false);
    /** Functions identical to sound(..., true). */
    void ambient_sound(int x, int y, int vol, std::string description);
    void ambient_sound(tripoint p, int vol, std::string description);
    ...
    void add_footstep(tripoint p, int volume, int distance, monster *source);
