 ...
 * window (via @ref delwin_functor) when the variable gets out of scope.
 * This includes calling @ref werase, @ref wrefresh and @ref delwin.
 *
 * Usage:
 ...
 * 3. Do the usual stuff with window, print, update, etc. but do *not* call @ref delwin on it.
...
/**
 * @defgroup color_tags color tags
 *
 ...
 *
 * Color tags allow to embed coloring information directly into the text. Therefor one can
 * call the print function only once with the full text. The color tags are removed
 ...
 * not contain any color tags.
 *
 * Functions that handle color tags should contain a note about this in their documentation. If
 * they have no such note, they probably don't handle color tags (which means the just print the
 * string as is).
 *
