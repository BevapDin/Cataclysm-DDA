...
{
    turn_number = to_turns<int>( time_duration::from_minutes( Minute ) ) + to_turns<int>
                  ( time_duration::from_hours( Hour ) ) + to_turns<int>( time_duration::from_days(
                              Day ) ) + Season * to_days<int>
...
time_point sunrise( const time_point &p )
{
    ...
    const time_duration time_into_season = ( p - calendar::time_of_cataclysm ) %
...
{
    return time_duration( rng( lo.moves_, hi.moves_ ) );
}
