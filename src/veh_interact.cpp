...
task_reason veh_interact::cant_do (char mode)
{
    ...
    case 's': // siphon mode
        ...
        for( const vpart_reference vp : veh->get_parts_including_broken( VPFLAG_FLUIDTANK ) ) {
            if( vp.part().base.contents.front().made_of( LIQUID ) ) {
                valid_target = true;
                break;
            }
        }
...
void act_vehicle_siphon( vehicle *veh ) {
    ...
    for( const vpart_reference vp : veh->get_parts_including_broken( VPFLAG_FLUIDTANK ) ) {
        if( vp.part().get_base().contents.front().made_of( LIQUID ) ) {
            has_liquid = true;
            break;
        }
    }
...
void veh_interact::complete_vehicle()
{
    ...
    case 'c':
        ...
        if( parts.size() ) {
            item removed_wheel;
            const auto replaced_wheel = veh->part_with_feature_including_broken( parts[0], "WHEEL" );
            if( !replaced_wheel ) {
                debugmsg( "no wheel to remove when changing wheels." );
                return;
            }
            const bool broken = replaced_wheel.part().is_broken();
            removed_wheel = replaced_wheel.part().properties_to_item();
            veh->remove_part( replaced_wheel.part_index() );
            veh->part_removal_cleanup();
