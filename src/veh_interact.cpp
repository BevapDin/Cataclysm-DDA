    ...
    : main_context("VEH_INTERACT")
{
    ...
    main_context.register_action("INVERT_CARGO_COLOR");
    main_context.register_action("HIDE_SOLAR_PANELS");
...
void veh_interact::do_main_loop()
{
    ...
    while (!finish) {
        ...
        if (main_context.get_direction(dx, dy, action)) {
            ...
            finish = true;
        } else if (action == "INVERT_CARGO_COLOR") {
            invert_cargo_color = !invert_cargo_color;
            display_veh();
        } else if (action == "HIDE_SOLAR_PANELS") {
...
void veh_interact::do_repair()
{
    ...
    while (true) {
        ...
        fold_and_print(w_msg, 0, 1, msg_width - 2, c_ltgray,
                       ...
                       has_skill ? "ltgreen" : "red",
                       dif);
        vpart_info::type_count_pair_vector items_needed = sel_vehicle_part->info().get_repair_materials(sel_vehicle_part->hp);
 ...
 */
item crafting_inventory_t::consume_vpart_item (const std::string &itid)
{
...
void complete_vehicle ()
{
    ...
    case 'i':
        ...
        used_item = crafting_inv.consume_vpart_item (part_id.obj().item);
    ...
    case 'r':
        ...
        if (veh->parts[vehicle_part].hp <= 0) {
            ...
            used_item = crafting_inv.consume_vpart_item (veh->parts[vehicle_part].info().item);
    ...
    case 'c':
        ...
        if( parts.size() ) {
            ...
            used_item = crafting_inv.consume_vpart_item( vpinfo.item );
            partnum = veh->install_part( dx, dy, part_id, used_item );
            if( partnum < 0 ) {
