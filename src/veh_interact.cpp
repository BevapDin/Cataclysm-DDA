...
void veh_interact::move_cursor( int dx, int dy, int dstart_at )
{
    ...
    if( !obstruct ) {
        ...
        for( const auto &e : vpart_info::all() ) {
            ...
            if( veh->can_mount( tripoint( vdx, vdy, 0 ), vp.get_id() ) ) {
...
void veh_interact::complete_vehicle()
{
    ...
    case 'i': {
        ...
        int partnum = !base.is_null() ? veh->install_part( tripoint( dx, dy, 0 ), part_id, std::move( base ) ) : -1;
        if(partnum < 0) {
    ...
    case 'c':
        ...
        if( parts.size() ) {
            ...
            veh->part_removal_cleanup();
            int partnum = veh->install_part( tripoint( dx, dy, 0 ), part_id, consume_vpart_item( part_id ) );
            if( partnum < 0 ) {
