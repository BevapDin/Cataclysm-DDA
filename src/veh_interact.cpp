...
void veh_interact::move_cursor( int dx, int dy, int dstart_at )
{
    ...
    if( !obstruct ) {
        ...
        for( const auto &e : vpart_info::all() ) {
            ...
            if( veh->can_mount( vdx, vdy, vp.get_id() ).success() ) {
...
void veh_interact::complete_vehicle()
{
    ...
    case 'i': {
        ...
        vehicle_part &new_part = veh->install_part( dx, dy, part_id, std::move( base ) );
        ...
        add_msg( m_good, _("You install a %1$s into the %2$s." ), new_part.name().c_str(), veh->name.c_str() );
    ...
    case 'c':
        ...
        if( parts.size() ) {
            ...
            veh->part_removal_cleanup();
            vehicle_part &new_part = veh->install_part( dx, dy, part_id, consume_vpart_item( part_id ) );
