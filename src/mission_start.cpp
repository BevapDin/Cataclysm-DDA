...
{
    npc *const p = miss->giver.get<npc>();
...
void mission_start::place_dog( mission *miss )
{
    ...
    npc *const dev = miss->giver.get<npc>();
    if( dev == NULL ) {
...
void mission_start::place_caravan_ambush( mission *miss )
{
    ...
    miss->target_npc = bay.place_npc( SEEX - 3, SEEY - 4, string_id<npc_template>( "bandit" ) );
    bay.save();
}
...
void mission_start::place_bandit_cabin( mission *miss )
{
    ...
    miss->target_npc = cabin.place_npc( SEEX, SEEY, string_id<npc_template>( "bandit" ) );
    cabin.save();
}
...
void mission_start::place_informant( mission *miss )
{
    ...
    tinymap bay;
    bay.load( site.x * 2, site.y * 2, site.z, false );
    miss->target_npc = bay.place_npc( SEEX, SEEY, string_id<npc_template>( "evac_guard3" ) );
    bay.save();
...
void mission_start::place_bandit_camp( mission *miss )
{
    ...
    miss->target_npc = bay1.place_npc( SEEX + 5, SEEY - 3, string_id<npc_template>( "bandit" ) );
    bay1.save();
}
...
{
    const npc *const p = miss->get_giver().get<npc>();
    if( p == nullptr ) {
        debugmsg( "mission_start::infect_npc() couldn't find an NPC!" );
        return;
    }
