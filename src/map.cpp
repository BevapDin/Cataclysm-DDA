                 ...
                 const bool low_light, const bool bright_light)
{
 ...
 if (has_flag("SWIMMABLE", x, y) && has_flag(TFLAG_DEEP_WATER, x, y) && !u.is_underwater()) {
  show_items = false; // Can only see underwater items if WE are underwater
    ...
    if (curr_trap != tr_null && curr_trap >= g->traps.size())
    {
        debugmsg("invalid trap id %d", (int) curr_trap);
    } else if (curr_trap != tr_null)
    {
        trap *t = g->traps[curr_trap];
        if (t->visibility == -1 ||
            u.per_cur - u.encumb(bp_eyes) >= t->visibility) {
            tercol = t->color;
            if (t->sym == '%') {
                static const char syms[5] = { '*', '0', '8', '&', '+' };
                sym = syms[rng(0, 4)];
            } else {
                sym = t->sym;
            }
