...
void map::bash_ter_furn( const tripoint &p, bash_params &params )
{
    ...
    if( has_flag("ALARMED", p) && !event_manager::get_instance().queued(EVENT_WANTED) ) {
        ...
        if( rl_dist( g->u.pos(), p ) <= 3 ) {
            ...
            event_manager::get_instance().add(EVENT_WANTED, int(calendar::turn) + 300, 0, tripoint( abs.x, abs.y, p.z ) );
        }
    }
...
void map::shoot( const tripoint &p, projectile &proj, const bool hit_items )
{
    ...
    if( has_flag("ALARMED", p) && !event_manager::get_instance().queued(EVENT_WANTED) ) {
        ...
        event_manager::get_instance().add(EVENT_WANTED, int(calendar::turn) + 300, 0, abs );
    }
