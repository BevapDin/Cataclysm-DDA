...
{
    return veh_part_coordinates( tripoint( x, y, 0 ) );
}
...
bool map::could_see_items(const tripoint &p, const player &u)
{
    ...
    if (container) {
        ...
        return (abs(p.x - u.posx()) <= 1 && abs(p.y - u.posy()) <= 1 && abs(p.z - u.posz()) <= 1);
    }
    return true;
}
...
{
    return i_rem( tripoint( location.x, location.y, 0 ), it );
}
...
{
    add_item(tripoint(x, y, 0), new_item);
}
...
{
    add_item_at( tripoint( x, y, 0 ), index, new_item );
}
...
static bool process_item( item_stack &items, Iterator &n, tripoint location, bool activate )
{
...
static bool process_map_items( item_stack &items, std::list<item>::iterator &n, tripoint location,
                               std::string )
{
...
static bool trigger_radio_item( item_stack &items, std::list<item>::iterator &n, tripoint pos,
                                std::string signal )
{
...
void map::add_camp( const std::string &name, const tripoint p )
{
    if (!allow_camp(p)) {
...
void map::draw(WINDOW* w, const tripoint &center)
{
 ...
 for  (real.x = center.x - getmaxx(w)/2; real.x <= center.x + getmaxx(w)/2; real.x++) {
  for (real.y = center.y - getmaxy(w)/2; real.y <= center.y + getmaxy(w)/2; real.y++) {
   ...
   // TODO: Z
   bool can_see = pl_sees( real, distance_to_look );
...
bool blocks_z( const map &m, int &last_z, const tripoint &pnt )
{
    if( pnt.z < last_z ) {
        if( m.blocks_vertical_view_up( pnt ) ) {
            return true;
        }
        last_z = pnt.z;
    } else if( pnt.z > last_z ) {
        if( m.blocks_vertical_view_down( pnt ) ) {
            return true;
        }
        last_z = pnt.z;
    }
...
bool blocks( const map &m, int &last_z, const tripoint &pnt )
{
    if( blocks_z( m, last_z, pnt ) ) {
        return true;
    } else if( m.light_transparency( pnt ) <= LIGHT_TRANSPARENCY_SOLID ) {
        return true;
    }
    return false;
}
...
bool map::sees( const tripoint F, const tripoint T, const int range, int &bresenham_slope ) const
{
    ...
    bresenham_slope = 0;
    // both points are valid, therefor all points between are valid and we don't
    // have to check that during the loop.
    ...
    if( range >= 0 ) {
        if( abs( dx ) > range || abs( dy ) > range || abs( dz ) > range ) {
            return false;
                }
                    }
    tripoint pnt = F;
    ...
    // Note: in-loop checks for crossing z-level *and* transparency, but last item checks
    // only z-level crossing because the final square is visible even when it's opaque (like a wall)
    if ((l >= m) && (l >= n)) {
        int err_1 = dy2 - l;
        int err_2 = dz2 - l;
        int last_z = pnt.z;
        for (int i = 0; i < l; i++) {
            if( blocks( *this, last_z, pnt ) ) {
                return false;
                    }
            if (err_1 > 0) {
                pnt.y += y_inc;
        ...
        bresenham_slope = err_1 * SGN(dx2 - (dy2 >> 1));
        return !blocks_z( *this, last_z, pnt );
    } else if ((m >= l) && (m >= n)) {
        int err_1 = dx2 - m;
        int err_2 = dz2 - m;
        int last_z = pnt.z;
...
bool map::clear_path(const tripoint &F, const tripoint &T,
                     const int range, const int cost_min, const int cost_max, int &bresenham_slope) const
{
    return clear_path(F.x, F.y, T.x, T.y, range, cost_min, cost_max, bresenham_slope);
}
...
{
    return accessible_items(F.x, F.y, T.x, T.y, range);
}
...
{
    return accessible_furniture(F.x, F.y, T.x, T.y, range);
}
...
void map::grow_plant( const tripoint pnt )
{
    ...
    if( items.empty() ) {
        ...
        dbg( D_ERROR ) << "a seed item has vanished at " << pnt;
        furn_set( pnt, f_null );
        return;
    }
    ...
    if ( calendar::turn >= seed.bday + plantEpoch ) {
		if (calendar::turn < seed.bday + plantEpoch * 2 ) {
				furn_set(pnt, "f_plant_seedling");
		} else if (calendar::turn < seed.bday + plantEpoch * 3 ) {
				furn_set(pnt, "f_plant_mature");
		} else {
				furn_set(pnt, "f_plant_harvest");
		}
	}
}
