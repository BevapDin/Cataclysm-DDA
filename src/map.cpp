...
void map::remove_vehicle (vehicle *veh)
{
 ...
 for (size_t i = 0; i < grid[veh_sm]->vehicles.size(); i++) {
  if (grid[veh_sm]->vehicles[i] == veh) {
   get_cache( veh->smz ).vehicle_list.erase(veh);
   reset_vehicle_cache( veh->smz );
...
void map::insert_vehicle (vehicle *veh, const int x, const int y)
{
    ...
    veh->posx = x % SEEX;
    veh->posy = y % SEEY;
    const int nonant = get_nonant( veh->smx, veh->smy, veh->smz );
    grid[nonant]->vehicles.push_back(veh);
    get_cache( veh->smz ).vehicle_list.insert(veh);
                 ...
                 const bool hit_items, const std::set<std::string>& ammo_effects )
{
    ...
    if (veh)
    {
        if (ammo_effects.count("LASER") && light_transparency(p) > LIGHT_TRANSPARENCY_SOLID) {
            dam -= rng(0, 5);
        } else {
