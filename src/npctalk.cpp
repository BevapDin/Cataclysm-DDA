...
void npc::talk_to_u()
{
    ...
    d.win = catacurses::window( FULL_SCREEN_HEIGHT, FULL_SCREEN_WIDTH,
...
bool trade( npc &p, int cost, const std::string &deal )
{
    catacurses::window w_head( 4, TERMX, 0, 0 );
    const int win_they_w = TERMX / 2;
    catacurses::window w_them( TERMY - 4, win_they_w, 4, 0 );
    catacurses::window w_you( TERMY - 4, TERMX - win_they_w, 4, win_they_w );
...
? to get information on an item." );
    ...
    do {
        ...
        switch( ch ) {
            ...
            case '?':
                update = true;
                w_tmp = catacurses::window( 3, 21, 1 + ( TERMY - FULL_SCREEN_HEIGHT ) / 2,
