...
void npc::move()
{
    ...
    if( sees_dangerous_field( point( posx(), posy() ) ) ) {
        auto targets = closest_points_first( 1, point( posx(), posy() ) );
...
void npc::execute_action(npc_action action, int target)
{
    ...
    */
    ...
    case npc_flee:
        ...
        move_away_from(targ.x, targ.y);
        break;

    case npc_melee:
        update_path(targ.x, targ.y);
        if (path.size() > 1) {
            move_to_next();
        } else if (path.size() == 1) {
            if (target >= 0) {
...
bool npc::wont_hit_friend(int tarx, int tary, int weapon_index)
{
...
void npc::alt_attack(int target)
{
    ...
    if (!thrown_item(used)) {
        ...
        if (dist <= confident_range(weapon_index) && wont_hit_friend(targ.x, targ.y, weapon_index)) {

            if (g->m.sees(pos(), targ, light, linet)) {
                trajectory = line_to(pos(), targ, linet);
            } else {
                trajectory = line_to(pos(), targ, 0);
            }
            moves -= 125;
            if (g->u.sees( *this )) {
            ...
            g->throw_item(*this, targ, *used, trajectory);
        ...
        } else if (!wont_hit_friend(targ.x, targ.y, weapon_index)) {// Danger of friendly fire
                    ...
                    for (int x = posx() - dist; x <= posx() + dist; x++) {
                        for (int y = posy() - dist; y <= posy() + dist; y++) {
                            ...
                            if (newdist <= conf && newdist >= 2 && newtarget != -1 &&
                                wont_hit_friend(x, y, weapon_index)) { // Friendlyfire-safe!
                ...
                for (int dist = 2; dist <= conf; dist++) {
                    for (int x = posx() - dist; x <= posx() + dist; x++) {
                        for (int y = posy() - dist; y <= posy() + dist; y++) {
                            ...
                            if (new_dist > best_dist && wont_hit_friend(x, y, weapon_index)) {
                                best_dist = new_dist;
                                targ.x = x;
                                targ.y = y;
                            }
