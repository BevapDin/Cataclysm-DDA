...
function generate_getter(class_name, member_name, member_type, cpp_name, static)
    ...
    if static then
        text = text .. tab .. push_lua_value(class_name .. '::' .. cpp_name, member_type .. "&")..';'..br
    else
...
function make_iterator_class(container_type, element_type)
    ...
    classes[iterator_type] = {
        ...
        functions = {
            { name = "elem", rval = element_type .. "&", cpp_name = "operator*", args = { } },
...
-- Create a class definitions for the C++ type `std::list<element_type>`.
-- The definition exports most of the functions of the `std::list` template,
-- but note that `end` and `begin` are keywords in Lua and those functions are
-- exported as `cppend` and `cppbegin`.
function make_list_class(element_type)
...
-- Same as make_list_class, but for `std::vector`.
