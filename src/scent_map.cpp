...
int scent_map::get( const tripoint &p ) const
{
    if( inbounds( p ) && grscent[p.x][p.y] > 0 ) {
        return grscent[p.x][p.y] - std::abs( gm.m.get_abs_sub().z - p.z );
    }
    return 0;
}
...
bool scent_map::inbounds( const tripoint &p ) const
{
    ...
    return p.x >= 0 && p.x < SEEX * MAPSIZE && p.y >= 0 && p.y < SEEY * MAPSIZE &&
           ( p.z == gm.m.get_abs_sub().z || ( std::abs( p.z - gm.m.get_abs_sub().z ) == 1 &&
                                       gm.m.valid_move( p, tripoint( p.x, p.y, gm.m.get_abs_sub().z ), false, true ) ) );
}
