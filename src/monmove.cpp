...
bool monster::can_move_to(const tripoint &p) const
{
    ...
    } else if (p.z < posz()) { // moving downwards
        // TODO: Z allow jumping down
        if (!g->m.has_flag_ter("GOES_DOWN", tripoint(p.x, p.y, posz()))) {
...
tripoint monster::wander_next()
{
    ...
    const tripoint n1(x, y, posz());
    const tripoint n2(x, y2, posz());
    const tripoint n3(x2, y, posz());
    const tripoint n4(x, y3, posz());
    const tripoint n5(x3, y, posz());
