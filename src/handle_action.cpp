    ...
    if( const optional_vpart_position vp = m.veh_at( openp ) ) {
        ...
        if( openable >= 0 ) {
            ...
            if( !outside ) {
                ...
                // curtains as well.
                int outside_openable = vp->vehicle().next_part_to_open( vp->part_index(), true );
                if( outside_openable == -1 ) {
                    const std::string name = vp->vehicle().part_info( openable ).name();
