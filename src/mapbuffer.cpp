...
{
    const std::string map_directory = world_generator->active_world->world_path + "/maps";
    ...
    for( auto &elem : submaps ) {
        ...
        const std::string dirname = map_directory + "/" + string_format( "%d.%d.%d", segment_addr.x, segment_addr.y, segment_addr.z );
        const std::string quad_path = dirname + "/" + string_format( "%d.%d.%d.map", om_addr.x, om_addr.y, om_addr.z );
...
submap *mapbuffer::unserialize_submaps( const tripoint &p )
{
    ...
    const tripoint segment_addr = omt_to_seg_copy( om_addr );
    const std::string quad_path = world_generator->active_world->world_path + "/maps/" +
        string_format( "%d.%d.%d", segment_addr.x, segment_addr.y, segment_addr.z ) + "/" +
        string_format( "%d.%d.%d.map", om_addr.x, om_addr.y, om_addr.z );

    using namespace std::placeholders;
    if( !read_from_file_optional_json( quad_path,
